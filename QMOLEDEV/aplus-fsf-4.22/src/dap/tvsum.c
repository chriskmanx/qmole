...
/*
 * p1 is added to p2 and the result is placed in p3. 0 is returned if the
 * operation succeeds.  1 is returned if a positive overflow was encountered
 * while preparing the result.  2 is returned if negative overflow was
 * encountered.  The inputs are assumed to be normalized.
 */
...
int 
tvsum(struct timeval * p1, struct timeval * p2, struct timeval * p3)
{
  static char fnc[] = "tvsum";
  ...
  long usec2;
{
  ...
  tv2.tv_sec = sec2;
  tv2.tv_usec = usec2;
  rc = tvsum(&tv1, &tv2, &tv3);
  if (printit) {
    (void) fprintf(stdout,
		   "tvsum((%ld, %ld), (%ld, %ld), (%ld, %ld)) = %d\n",
