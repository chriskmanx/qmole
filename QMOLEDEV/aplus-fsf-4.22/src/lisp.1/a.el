;;; a.el -- A+ major mode and utilities.
...
(defvar a-mode-saved-syntax-table nil "Saved syntax table for toggling back.")
(make-variable-buffer-local 'a-mode-saved-syntax-table)
...
(defun unset-a-comment-parms ()
  "Undo assignation of a+ comment parameters"
  (setq comment-multi-line nil)
  (setq comment-start (char-to-string ?\343))
  (setq comment-end "")
  (setq comment-column 1)
...
(defvar a-mode-hook nil
  "*Hook called by a-mode.")
...
(defun a-mode ()
  ...
  (setq mode-name "A")
  (set-face-font 'default a+-font (current-buffer))
  (auto-fill-mode -1)
...
(defun toggle-a-minor-mode ()
  ...
  (if a-minor 
	...
	(setq a-mode-saved-syntax-table (syntax-table))
	(set-syntax-table a-mode-syntax-table)
...
;	(setq lpr-switches apr-switches)
    ...
    (progn 
      ...
      (set-syntax-table a-mode-saved-syntax-table)
      (remove-face-property 'default 'font (current-buffer))
...
(defvar a-mode-fix-my-modeline t
  "Fix my modeline so that I know I use A+.")
(if a-mode-fix-my-modeline
    (progn
...
;;;=========================================================================
;; The isearch needs to be fixed, but this isn't it. -- Colin
...
(defvar a-mode-fix-gdb t
  "Fix GDB because I want to use A+.")
...
;; Anyway, this should be remove-hook, not setq.
(defvar a-mode-fix-toggle t
  "Fix C-x C-q because I want to use A+.")

(if a-mode-fix-toggle
    (progn
