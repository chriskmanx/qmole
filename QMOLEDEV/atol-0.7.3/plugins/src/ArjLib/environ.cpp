...
int
file_seek(FILE *stream, long offset, int  mode)
{
    return fseek(stream, offset, mode);
}

long
file_tell(FILE *stream)
{
    return ftell(stream);
}

int
file_write(char *buf, int size, int nitems, FILE *stream)
{
    return fwrite(buf, (int) size, (int) nitems, stream);
}
...
void
get_mode_str(char *str, uint mode)
{
    strcpy(str, "---W");
    if (mode & FA_ARCH)
        str[0] = 'A';
...
int
set_ftime_mode(char *name, ulong tstamp, uint attribute, uint host)
{
    (char *) name;
    (ulong) tstamp;
    (uint) attribute;
