...
bool g_bShowDirWarningDialog = true;
...
DeleteDirDlg::~DeleteDirDlg()
{
	...
	if(m_bHideDialog){
		IniFile ini;
		ini.Load(GetIniFile());
		ini.SetValue("Operation", "DeleteDirWarning", m_bHideDialog);
		ini.Save();
	}
	g_bShowDirWarningDialog = !m_bHideDialog;
}

void DeleteDirDlg::SetInfo(const char *szTxt)
{
...
GtkWidget* DeleteDirDlg::create_dialog()
{
...
{
	gtk_dialog_response(GTK_DIALOG(user_data), OPF_DEL_ALL_DIRS);
}
...
{
	DeleteDirDlg *pDlg = (DeleteDirDlg *)user_data;
	GtkWidget *hide_check = lookup_widget(pDlg->m_pDialog, "checkbutton15");
	pDlg->m_bHideDialog = (0 != gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(hide_check)));
}
