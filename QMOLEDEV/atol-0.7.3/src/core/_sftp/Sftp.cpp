...
static void sftp_pkt_ensure(struct sftp_packet *pkt, int length)
{
	...
	pkt->data = (char *)srealloc(pkt->data, pkt->maxlen);
    }
}
...
static struct sftp_packet *sftp_pkt_init(int pkt_type)
{
    ...
    pkt = (struct sftp_packet *)smalloc(sizeof(struct sftp_packet));
    pkt->data = NULL;
    pkt->savedpos = -1;
    pkt->length = 0;
    pkt->maxlen = 0;
 ...
 */
int sftp_send(CSshSession &session, struct sftp_packet *pkt)
{
    int ret;
    char x[4];
    PUT_32BIT(x, pkt->length);
    ret = (sftp_senddata(session, x, 4) && sftp_senddata(session, pkt->data, pkt->length));
    sftp_pkt_free(pkt);
    return ret;
}

struct sftp_packet *sftp_recv(CSshSession &session)
{
    struct sftp_packet *pkt;
    char x[4];

    if (!sftp_recvdata(session, x, 4))
		return NULL;

    pkt = (struct sftp_packet *)smalloc(sizeof(struct sftp_packet));
    pkt->savedpos = 0;
    pkt->length = pkt->maxlen = GET_32BIT(x);
    pkt->data = (char *)smalloc(pkt->length);

    if (!sftp_recvdata(session, pkt->data, pkt->length)) {
		sftp_pkt_free(pkt);
		return NULL;
    }
 ...
 */
int fxp_init(CSshSession &session)
{
    ...
    sftp_send(session, pktout);

    pktin = sftp_recv(session);
    if (!pktin) {
 ...
 * Canonify a pathname.
 */
char *fxp_realpath(CSshSession &session, char *path)
{
    ...
    if (pktin->type == SSH_FXP_NAME) {
		...
		sftp_pkt_getstring(pktin, &path1, &len);
		if (!path1) {
			fxp_internal_error("REALPATH returned malformed FXP_NAME\n");
			sftp_pkt_free(pktin);
			return NULL;
		}
		path1 = mkstr(path1, len);
		sftp_pkt_free(pktin);
		return path1;
	} else {
 ...
 * Open a file.
 */
struct fxp_handle *fxp_open(CSshSession &session, char *path, int type)
{
    struct sftp_packet *pktin, *pktout;
    int id;
    ...
    if (pktin->type == SSH_FXP_HANDLE) {
		...
		handle = (struct fxp_handle *)smalloc(sizeof(struct fxp_handle));
 ...
 * Open a directory.
 */
struct fxp_handle *fxp_opendir(CSshSession &session, char *path)
{
    struct sftp_packet *pktin, *pktout;
    int id;
 ...
 * Close a file/dir.
 */
void fxp_close(CSshSession &session, struct fxp_handle *handle)
{
    struct sftp_packet *pktin, *pktout;
    int id;
...
int fxp_mkdir(CSshSession &session, char *path)
{
    struct sftp_packet *pktin, *pktout;
    int id;
...
int fxp_rmdir(CSshSession &session, char *path)
{
    struct sftp_packet *pktin, *pktout;
    int id;
...
int fxp_remove(CSshSession &session, char *fname)
{
    struct sftp_packet *pktin, *pktout;
    int id;
...
int fxp_rename(CSshSession &session, char *srcfname, char *dstfname)
{
    struct sftp_packet *pktin, *pktout;
    int id;
 ...
 */
int fxp_stat(CSshSession &session, char *fname, struct fxp_attrs *attrs)
{
    struct sftp_packet *pktin, *pktout;
    int id;
...
int fxp_fstat(CSshSession &session, struct fxp_handle *handle, struct fxp_attrs *attrs)
{
    struct sftp_packet *pktin, *pktout;
    int id;
 ...
 */
int fxp_setstat(CSshSession &session, char *fname, struct fxp_attrs attrs)
{
    struct sftp_packet *pktin, *pktout;
    int id;
...
int fxp_fsetstat(CSshSession &session, struct fxp_handle *handle, struct fxp_attrs attrs)
{
    struct sftp_packet *pktin, *pktout;
    int id;
 ...
 */
int fxp_read(CSshSession &session, struct fxp_handle *handle, char *buffer, uint64 offset,
	     int len)
{
    struct sftp_packet *pktin, *pktout;
    int id;
 ...
 */
struct fxp_names *fxp_readdir(CSshSession &session, struct fxp_handle *handle)
{
    struct sftp_packet *pktin, *pktout;
    int id;
    ...
    if (pktin->type == SSH_FXP_NAME) {
		...
		int i;
		ret = (struct fxp_names *)smalloc(sizeof(struct fxp_names));
		ret->nnames = sftp_pkt_getuint32(pktin);
		ret->names = (struct fxp_name *)smalloc(ret->nnames * sizeof(struct fxp_name));
 ...
 */
int fxp_write(CSshSession &session, struct fxp_handle *handle, char *buffer, uint64 offset,
	      int len)
{
    struct sftp_packet *pktin, *pktout;
    int id;
...
{
    struct fxp_name *ret;
    ret = (struct fxp_name *)smalloc(sizeof(struct fxp_name));
