...
static void aes128_cskey(CSshSession &session, unsigned char *key)
{
    aes_setup(&session.csctx, 16, key, 16);
    logevent(session, "Initialised AES-128 client->server encryption");
}

static void aes128_sckey(CSshSession &session, unsigned char *key)
{
    aes_setup(&session.scctx, 16, key, 16);
    logevent(session, "Initialised AES-128 server->client encryption");
}

static void aes192_cskey(CSshSession &session, unsigned char *key)
{
    aes_setup(&session.csctx, 16, key, 24);
    logevent(session, "Initialised AES-192 client->server encryption");
}

static void aes192_sckey(CSshSession &session, unsigned char *key)
{
    aes_setup(&session.scctx, 16, key, 24);
    logevent(session, "Initialised AES-192 server->client encryption");
}

static void aes256_cskey(CSshSession &session, unsigned char *key)
{
    aes_setup(&session.csctx, 16, key, 32);
    logevent(session, "Initialised AES-256 client->server encryption");
}

static void aes256_sckey(CSshSession &session, unsigned char *key)
{
    aes_setup(&session.scctx, 16, key, 32);
    logevent(session, "Initialised AES-256 server->client encryption");
}

static void aes_csiv(CSshSession &session, unsigned char *iv)
{
    int i;
    for (i = 0; i < 4; i++)
		session.csctx.iv[i] = GET_32BIT_MSB_FIRST(iv + 4 * i);
}

static void aes_sciv(CSshSession &session, unsigned char *iv)
{
    int i;
    for (i = 0; i < 4; i++)
		session.scctx.iv[i] = GET_32BIT_MSB_FIRST(iv + 4 * i);
}

static void aes_ssh2_encrypt_blk(CSshSession &session, unsigned char *blk, int len)
{
    aes_encrypt_cbc(blk, len, &session.csctx);
}

static void aes_ssh2_decrypt_blk(CSshSession &session, unsigned char *blk, int len)
{
    aes_decrypt_cbc(blk, len, &session.scctx);
}
