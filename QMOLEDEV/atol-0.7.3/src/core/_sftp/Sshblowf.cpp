...
static void blowfish_cskey(CSshSession &session, unsigned char *key)
{
    blowfish_setkey(&session.ectx, key, 16);
    logevent(session, "Initialised Blowfish client->server encryption");
}

static void blowfish_sckey(CSshSession &session, unsigned char *key)
{
    blowfish_setkey(&session.dctx, key, 16);
    logevent(session, "Initialised Blowfish server->client encryption");
}

static void blowfish_csiv(CSshSession &session, unsigned char *key)
{
    session.ectx.iv0 = GET_32BIT_MSB_FIRST(key);
    session.ectx.iv1 = GET_32BIT_MSB_FIRST(key + 4);
}

static void blowfish_sciv(CSshSession &session, unsigned char *key)
{
    session.dctx.iv0 = GET_32BIT_MSB_FIRST(key);
    session.dctx.iv1 = GET_32BIT_MSB_FIRST(key + 4);
}

static void blowfish_sesskey(CSshSession &session, unsigned char *key)
{
    blowfish_setkey(&session.ectx, key, SSH_SESSION_KEY_LENGTH);
    session.ectx.iv0 = 0;
    session.ectx.iv1 = 0;
    session.dctx = session.ectx;
    logevent(session, "Initialised Blowfish encryption");
}

static void blowfish_ssh1_encrypt_blk(CSshSession &session, unsigned char *blk, int len)
{
    blowfish_lsb_encrypt_cbc(blk, len, &session.ectx);
}

static void blowfish_ssh1_decrypt_blk(CSshSession &session, unsigned char *blk, int len)
{
    blowfish_lsb_decrypt_cbc(blk, len, &session.dctx);
}

static void blowfish_ssh2_encrypt_blk(CSshSession &session, unsigned char *blk, int len)
{
    blowfish_msb_encrypt_cbc(blk, len, &session.ectx);
}

static void blowfish_ssh2_decrypt_blk(CSshSession &session, unsigned char *blk, int len)
{
    blowfish_msb_decrypt_cbc(blk, len, &session.dctx);
}
