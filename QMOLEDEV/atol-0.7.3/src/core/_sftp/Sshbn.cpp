...
{
    Bignum b = (unsigned short *)smalloc((length + 1) * sizeof(unsigned short));
    if (!b)
		abort();		       /* FIXME */ //TOFIX
...
{
    Bignum b = (unsigned short *)smalloc((orig[0] + 1) * sizeof(unsigned short));
    if (!b)
		abort();		       /* FIXME */
...
Bignum modpow(Bignum base, Bignum exp, Bignum mod)
{
    ...
    mlen = mod[0];
    m = (unsigned short *)smalloc(mlen * sizeof(unsigned short));
    for (j = 0; j < mlen; j++)
	m[j] = mod[mod[0] - j];
    ...
    n = (unsigned short *)smalloc(mlen * sizeof(unsigned short));
    i = mlen - base[0];
    for (j = 0; j < i; j++)
	n[j] = 0;
    for (j = 0; j < base[0]; j++)
    ...
    a = (unsigned short *)smalloc(2 * mlen * sizeof(unsigned short));
    b = (unsigned short *)smalloc(2 * mlen * sizeof(unsigned short));
    for (i = 0; i < 2 * mlen; i++)
	a[i] = 0;
    a[2 * mlen - 1] = 1;
...
Bignum modmul(Bignum p, Bignum q, Bignum mod)
{
    ...
    n = (unsigned short *)smalloc(pqlen * sizeof(unsigned short));
    i = pqlen - p[0];
    for (j = 0; j < i; j++)
	n[j] = 0;
    for (j = 0; j < p[0]; j++)
    ...
    o = (unsigned short *)smalloc(pqlen * sizeof(unsigned short));
    i = pqlen - q[0];
    for (j = 0; j < i; j++)
	o[j] = 0;
    for (j = 0; j < q[0]; j++)
    ...
    a = (unsigned short *)smalloc(2 * pqlen * sizeof(unsigned short));
...
void bigdivmod(Bignum p, Bignum mod, Bignum result, Bignum quotient)
{
    ...
    n = (unsigned short *)smalloc(plen * sizeof(unsigned short));
    for (j = 0; j < plen; j++)
	n[j] = 0;
    for (j = 1; j <= p[0]; j++)
	n[plen - j] = p[j];
...
Bignum bigmuladd(Bignum a, Bignum b, Bignum addend)
{
    ...
    workspace = (unsigned short *)smalloc(mlen * 4 * sizeof(unsigned short));
    for (i = 0; i < mlen; i++) {
...
char *bignum_decimal(Bignum x)
{
     ...
     */
    workspace = (unsigned short *)smalloc(sizeof(unsigned short) * x[0]);
    for (i = 0; i < x[0]; i++)
		workspace[i] = x[x[0] - i];
