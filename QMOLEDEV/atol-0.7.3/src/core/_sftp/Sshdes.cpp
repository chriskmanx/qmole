...
static void des3_cskey(CSshSession &session, unsigned char *key)
{
    des_key_setup(GET_32BIT_MSB_FIRST(key),
		  GET_32BIT_MSB_FIRST(key + 4), &session.cskeys[0]);
    des_key_setup(GET_32BIT_MSB_FIRST(key + 8),
		  GET_32BIT_MSB_FIRST(key + 12), &session.cskeys[1]);
    des_key_setup(GET_32BIT_MSB_FIRST(key + 16),
		  GET_32BIT_MSB_FIRST(key + 20), &session.cskeys[2]);
    logevent(session, "Initialised triple-DES client->server encryption");
}

static void des_cskey(CSshSession &session, unsigned char *key)
{
    des_key_setup(GET_32BIT_MSB_FIRST(key),
		  GET_32BIT_MSB_FIRST(key + 4), &session.cskeys[0]);
    logevent(session, "Initialised single-DES client->server encryption");
}

static void des3_csiv(CSshSession &session, unsigned char *key)
{
    session.cskeys[0].eiv0 = GET_32BIT_MSB_FIRST(key);
    session.cskeys[0].eiv1 = GET_32BIT_MSB_FIRST(key + 4);
}

static void des3_sciv(CSshSession &session, unsigned char *key)
{
    session.sckeys[0].div0 = GET_32BIT_MSB_FIRST(key);
    session.sckeys[0].div1 = GET_32BIT_MSB_FIRST(key + 4);
}

static void des3_sckey(CSshSession &session, unsigned char *key)
{
    des_key_setup(GET_32BIT_MSB_FIRST(key),
		  GET_32BIT_MSB_FIRST(key + 4), &session.sckeys[0]);
    des_key_setup(GET_32BIT_MSB_FIRST(key + 8),
		  GET_32BIT_MSB_FIRST(key + 12), &session.sckeys[1]);
    des_key_setup(GET_32BIT_MSB_FIRST(key + 16),
		  GET_32BIT_MSB_FIRST(key + 20), &session.sckeys[2]);
    logevent(session, "Initialised triple-DES server->client encryption");
}

static void des_sckey(CSshSession &session, unsigned char *key)
{
    des_key_setup(GET_32BIT_MSB_FIRST(key),
		  GET_32BIT_MSB_FIRST(key + 4), &session.sckeys[0]);
    logevent(session, "Initialised single-DES server->client encryption");
}

static void des3_sesskey(CSshSession &session, unsigned char *key)
{
    des3_cskey(session, key);
    des3_sckey(session, key);
}

static void des3_encrypt_blk(CSshSession &session, unsigned char *blk, int len)
{
    des_3cbc_encrypt(blk, blk, len, session.cskeys);
}

static void des3_decrypt_blk(CSshSession &session, unsigned char *blk, int len)
{
    des_3cbc_decrypt(blk, blk, len, session.sckeys);
}

static void des3_ssh2_encrypt_blk(CSshSession &session, unsigned char *blk, int len)
{
    des_cbc3_encrypt(blk, blk, len, session.cskeys);
}

static void des3_ssh2_decrypt_blk(CSshSession &session, unsigned char *blk, int len)
{
    des_cbc3_decrypt(blk, blk, len, session.sckeys);
}

static void des_ssh2_encrypt_blk(CSshSession &session, unsigned char *blk, int len)
{
    des_cbc_encrypt(blk, blk, len, session.cskeys);
}

static void des_ssh2_decrypt_blk(CSshSession &session, unsigned char *blk, int len)
{
    des_cbc_decrypt(blk, blk, len, session.sckeys);
}
...
static void des_sesskey(CSshSession &session, unsigned char *key)
{
    des_cskey(session, key);
    des_sckey(session, key);
}

static void des_encrypt_blk(CSshSession &session, unsigned char *blk, int len)
{
    des_cbc_encrypt(blk, blk, len, session.cskeys);
}

static void des_decrypt_blk(CSshSession &session, unsigned char *blk, int len)
{
    des_cbc_decrypt(blk, blk, len, session.cskeys);
}
