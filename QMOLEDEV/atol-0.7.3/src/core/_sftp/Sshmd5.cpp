...
static void md5_cskey(CSshSession &session, unsigned char *key)
{
    md5_key(&session.md5_cs_mac_s1, &session.md5_cs_mac_s2, key, 16);
}

static void md5_sckey(CSshSession &session, unsigned char *key)
{
    md5_key(&session.md5_sc_mac_s1, &session.md5_sc_mac_s2, key, 16);
}
...
static void md5_generate(CSshSession &session, unsigned char *blk, int len, unsigned long seq)
{
    md5_do_hmac(&session.md5_cs_mac_s1, &session.md5_cs_mac_s2, blk, len, seq, blk + len);
}

static int md5_verify(CSshSession &session, unsigned char *blk, int len, unsigned long seq)
{
    unsigned char correct[16];
    md5_do_hmac(&session.md5_sc_mac_s1, &session.md5_sc_mac_s2, blk, len, seq, correct);
