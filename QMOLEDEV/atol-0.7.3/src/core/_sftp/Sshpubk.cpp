			   ...
			   char **commentptr, char *passphrase)
{
	...
	*commentptr = comment;
    ...
    if (ciphertype) {
	MD5Init(&md5c);
	MD5Update(&md5c, (const unsigned char *)passphrase, strlen(passphrase));
...
int loadrsakey(char *filename, struct RSAKey *key, char *passphrase)
{
     ...
     * key file.
     */
    if (fgets((char *)buf, sizeof(buf), fp) && !strcmp((const char *)buf, rsa_signature)) {
...
static char *read_blob(FILE * fp, int nlines, int *bloblen)
{
    ...
    blob = (unsigned char *)smalloc(48 * nlines);
    len = 0;
    for (i = 0; i < nlines; i++) {
	line = read_body(fp);
	if (!line) {
...
struct ssh2_userkey *ssh2_load_userkey(char *filename, char *passphrase)
{
    ...
    i = atoi(b);
    sfree(b);
    if ((public_blob = (unsigned char *)read_blob(fp, i, &public_blob_len)) == NULL)
	goto error;
    ...
    i = atoi(b);
    sfree(b);
    if ((private_blob = (unsigned char *)read_blob(fp, i, &private_blob_len)) == NULL)
	goto error;
	...
	if (old_fmt) {
	    ...
	    maclen = (4 + namelen +
		      ...
		      4 + public_blob_len +
		      4 + private_blob_len);
	    macdata = (unsigned char *)smalloc(maclen);
	    p = macdata;
#define DO_STR(s,len) PUT_32BIT(p,(len));memcpy(p+4,(s),(len));p+=4+(len)
     ...
     */
    ret = (struct ssh2_userkey *)smalloc(sizeof(struct ssh2_userkey));
    ret->alg = alg;
    ret->comment = comment;
		      ...
		      char *passphrase)
{
    ...
    priv_blob_encrypted = (unsigned char *)smalloc(priv_encrypted_len);
...
#define DO_STR(s,len) PUT_32BIT(p,(len));memcpy(p+4,(s),(len));p+=4+(len)
	...
	SHA_Bytes(&s, "\0\0\0\0", 4);
	SHA_Bytes(&s, passphrase, passlen);
	SHA_Final(&s, (unsigned char *)key + 0);
	SHA_Init(&s);
	SHA_Bytes(&s, "\0\0\0\1", 4);
	SHA_Bytes(&s, passphrase, passlen);
	SHA_Final(&s, (unsigned char *)key + 20);
	aes256_encrypt_pubkey((unsigned char *)key, priv_blob_encrypted,
			      priv_encrypted_len);
