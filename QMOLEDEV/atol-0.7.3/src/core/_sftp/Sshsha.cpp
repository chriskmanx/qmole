...
static void sha1_cskey(CSshSession &session, unsigned char *key)
{
    sha1_key(&session.sha1_cs_mac_s1, &session.sha1_cs_mac_s2, key, 20);
}

static void sha1_sckey(CSshSession &session, unsigned char *key)
{
    sha1_key(&session.sha1_sc_mac_s1, &session.sha1_sc_mac_s2, key, 20);
}

static void sha1_cskey_buggy(CSshSession &session, unsigned char *key)
{
    sha1_key(&session.sha1_cs_mac_s1, &session.sha1_cs_mac_s2, key, 16);
}

static void sha1_sckey_buggy(CSshSession &session, unsigned char *key)
{
    sha1_key(&session.sha1_sc_mac_s1, &session.sha1_sc_mac_s2, key, 16);
}
...
static void sha1_generate(CSshSession &session, unsigned char *blk, int len, unsigned long seq)
{
    sha1_do_hmac(&session.sha1_cs_mac_s1, &session.sha1_cs_mac_s2, blk, len, seq,
		 blk + len);
}

static int sha1_verify(CSshSession &session, unsigned char *blk, int len, unsigned long seq)
{
    unsigned char correct[20];
    sha1_do_hmac(&session.sha1_sc_mac_s1, &session.sha1_sc_mac_s2, blk, len, seq, correct);
