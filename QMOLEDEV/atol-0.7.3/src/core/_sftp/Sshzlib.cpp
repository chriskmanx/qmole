...
static void outbits(struct Outbuf *out, unsigned long bits, int nbits)
{
	...
	if (out->outlen >= out->outsize) {
	    ...
	    out->outbuf = (unsigned char *)srealloc(out->outbuf, out->outsize);
	}
...
void zlib_compress_init(CSshSession &session)
{
    ...
    out = (struct Outbuf *)smalloc(sizeof(struct Outbuf));
    out->outbits = out->noutbits = 0;
    out->firstblock = 1;
    out->comp_disabled = FALSE;
    ectx.userdata = out;

    logevent(session, "Initialised zlib (RFC1950) compression");
}
...
{
    struct zlib_table *tab = (struct zlib_table *)smalloc(sizeof(struct zlib_table));
    int pfxmask = (1 << pfxbits) - 1;
    int nbits, i, j, code;

    tab->table = (struct zlib_tableentry *)smalloc((1 << bits) * sizeof(struct zlib_tableentry));
    tab->mask = (1 << bits) - 1;
...
//  enum {START, OUTSIDEBLK,TREES_HDR, TREES_LENLEN, TREES_LEN, TREES_LENREP,
//	INBLK, GOTLENSYM, GOTLEN, GOTDISTSYM,UNCOMP_LEN, UNCOMP_NLEN, UNCOMP_DATA } state;

#define START			0
#define OUTSIDEBLK		1
#define TREES_HDR		2
#define TREES_LENLEN	3
#define TREES_LEN		4
#define TREES_LENREP	5
#define INBLK			6
#define GOTLENSYM		7
#define GOTLEN			8
#define GOTDISTSYM		9
#define UNCOMP_LEN		10
#define UNCOMP_NLEN		11
#define UNCOMP_DATA		12
...
void zlib_decompress_init(CSshSession &session)
{
    ...
    dctx.bits = 0;
    dctx.nbits = 0;
    logevent(session, "Initialised zlib (RFC1950) decompression");
}
...
static void zlib_emit_char(int c)
{
	...
	dctx.outblk = (unsigned char *)srealloc(dctx.outblk, dctx.outsize);
    }
