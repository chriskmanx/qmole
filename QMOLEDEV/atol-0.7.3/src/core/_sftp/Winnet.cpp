...
void sk_init(CSshSession &session)
{
    session.sktree = newtree234(cmpfortree);
}

void sk_cleanup(CSshSession &session)
{
    Actual_Socket s;
    int i;

    if (session.sktree) {
		for (i = 0; (s = (struct Socket_tag *)index234(session.sktree, i)) != NULL; i++) {
			closesocket(s->s);
		}
		freetree234(session.sktree); //MIRO
		session.sktree = NULL;
    }
}
...
SockAddr sk_namelookup(char *host, char **canonicalname)
{
    ...
    *canonicalname = (char *)smalloc(1+strlen(realhost));
...
static void sk_tcp_flush(CSshSession &session, Socket s)
{
    /*
...
static void sk_tcp_close(CSshSession &session, Socket s);
static int sk_tcp_write(CSshSession &session, Socket s, char *data, int len);
static int sk_tcp_write_oob(CSshSession &session, Socket s, char *data, int len);
static void sk_tcp_set_private_ptr(CSshSession &session, Socket s, void *ptr);
static void *sk_tcp_get_private_ptr(CSshSession &session, Socket s);
static void sk_tcp_set_frozen(CSshSession &session, Socket s, int is_frozen);
static char *sk_tcp_socket_error(CSshSession &session, Socket s);
...
Socket sk_register(CSshSession &session, void *sock, Plug plug)
{
     ...
     */
    ret = (struct Socket_tag *)smalloc(sizeof(struct Socket_tag));
    ret->fn = &fn_table;
    ret->error = NULL;
    ret->plug = plug;
    ...
    errstr = do_select(session, ret->s, 1);
    if (errstr) {
		ret->error = errstr;
		return (Socket) ret;
    }

    add234(session.sktree, ret);
	      ...
	      int nodelay, Plug plug)
{
    ...
    if (oobinline) {
	BOOL b = TRUE;
	setsockopt(s, SOL_SOCKET, SO_OOBINLINE, (const char *) &b, sizeof(b));
    }
...
static void sk_tcp_close(CSshSession &session, Socket sock)
{
    char *do_select(CSshSession &session, SOCKET skt, int startup);
    Actual_Socket s = (Actual_Socket) sock;

    del234(session.sktree, s);
    do_select(session, s->s, 0);
    closesocket(s->s);
    sfree(s);
}
 ...
 * writable.
 */
void try_send(CSshSession &session, Actual_Socket s)
{
	...
	nsent = send(s->s, (const char *)data, len, urgentflag);
	...
	if (nsent <= 0) {
		...
		logevent(session, winsock_error_string(err));
		fatalbox(session, "%s", winsock_error_string(err));
	    }
	} else {
	    if (s->sending_oob) {
		if (nsent < len) {
...
static int sk_tcp_write(CSshSession &session, Socket sock, char *buf, int len)
{
    Actual_Socket s = (Actual_Socket) sock;
...
static int sk_tcp_write_oob(CSshSession &session, Socket sock, char *buf, int len)
{
    Actual_Socket s = (Actual_Socket) sock;
...
int select_result(CSshSession &session, WPARAM wParam, LPARAM lParam)
{
    int ret, open;
    DWORD err;
    ...
    s = (struct Socket_tag *)find234(session.sktree, (void *) wParam, cmpforsearch);
    if (!s)
	return 1;		       /* boggle */
	 ...
	 * plug.
	 */
		return plug_closing(session, s->plug, winsock_error_string(err), err, 0);
    }
	...
	if (ret < 0) {
	    return plug_closing(session, s->plug, winsock_error_string(err), err,
				0);
	} else if (0 == ret) {
	    return plug_closing(session, s->plug, NULL, 0, 0);
	} else {
	    return plug_receive(session, s->plug, atmark ? 0 : 1, buf, ret);
	}
	break;
      case FD_OOB:
	/*
	...
	if (ret <= 0) {
	    ...
	    logevent(session, str);
	    fatalbox(session, "%s", str);
	} else {
	    return plug_receive(session, s->plug, 2, buf, ret);
	}
	break;
      case FD_WRITE:
	{
	    ...
	    if (bufsize_after < bufsize_before)
			plug_sent(session, s->plug, bufsize_after);
	}
	break;
      case FD_CLOSE:
	...
	do {
		...
		if (err == WSAEWOULDBLOCK)
		    break;
		return plug_closing(session, s->plug, winsock_error_string(err),
				    err, 0);
	    } else {
		if (ret)
		    open &= plug_receive(session,s->plug, 0, buf, ret);
		else
		    open &= plug_closing(session, s->plug, NULL, 0, 0);
	    }
	} while (ret > 0);
	return open;
       case FD_ACCEPT:
	{
	    ...
	    } else if (plug_accepting(session, s->plug, (void*)t)) {
 ...
 */
void net_pending_errors(CSshSession &session)
{
    int i;
    Actual_Socket s;
 ...
 */
static void sk_tcp_set_private_ptr(CSshSession &session, Socket sock, void *ptr)
{
    Actual_Socket s = (Actual_Socket) sock;
    s->private_ptr = ptr;
}

static void *sk_tcp_get_private_ptr(CSshSession &session, Socket sock)
{
    Actual_Socket s = (Actual_Socket) sock;
...
static char *sk_tcp_socket_error(CSshSession &session, Socket sock)
{
    Actual_Socket s = (Actual_Socket) sock;
    return s->error;
}

static void sk_tcp_set_frozen(CSshSession &session, Socket sock, int is_frozen)
{
    Actual_Socket s = (Actual_Socket) sock;
 ...
 */
SOCKET first_socket(CSshSession &session, int *state)
{
    Actual_Socket s;
    *state = 0;
    s = (struct Socket_tag *)index234(session.sktree, (*state)++);
    return s ? s->s : INVALID_SOCKET;
}

SOCKET next_socket(CSshSession &session, int *state)
{
    Actual_Socket s = (struct Socket_tag *)index234(session.sktree, (*state)++);
    return s ? s->s : INVALID_SOCKET;
}
