...
void *open_settings_w(char *sessionname)
{
    ...
    p = (char *)smalloc(3 * strlen(sessionname) + 1);
...
{
    if (handle)
	RegSetValueEx((HKEY) handle, key, 0, REG_SZ, (const unsigned char *)value,
		      1 + strlen(value));
}
...
char *read_setting_s(void *handle, char *key, char *buffer, int buflen)
{
	...
	RegQueryValueEx((HKEY) handle, key, 0,
			&type, (unsigned char *)buffer, &size) != ERROR_SUCCESS ||
...
void *enum_settings_start(void)
{
    ...
    ret = (struct enumsettings *)smalloc(sizeof(*ret));
    if (ret) {
	ret->key = key;
	ret->i = 0;
    }
...
char *enum_settings_next(void *handle, char *buffer, int buflen)
{
    ...
    char *otherbuf;
    otherbuf = (char *)smalloc(3 * buflen);
...
int verify_host_key(char *hostname, int port, char *keytype, char *key)
{
     ...
     * says.
     */
    otherstr = (char *)smalloc(len);
    regname = (char *)smalloc(3 * (strlen(hostname) + strlen(keytype)) + 15);
	...
	char *oldstyle = (char *)smalloc(len + 10);	/* safety margin */
	readlen = len;
	ret = RegQueryValueEx(rkey, justhost, NULL, &type,
			      (unsigned char *)oldstyle, &readlen);
	...
	if (ret == ERROR_SUCCESS && type == REG_SZ) {
		...
		*p = '\0';	       /* terminate the string */
		...
		RegSetValueEx(rkey, regname, 0, REG_SZ, (unsigned char *)otherstr,
			      strlen(otherstr) + 1);
	}
    }
...
void store_host_key(char *hostname, int port, char *keytype, char *key)
{
    ...
    RegSetValueEx(rkey, regname, 0, REG_SZ, (unsigned char *)key, strlen(key) + 1);
    RegCloseKey(rkey);
}
...
static void get_seedpath(void)
{
	...
	int ret = RegQueryValueEx(rkey, "RandSeedFile",
				  0, &type, (unsigned char *)seedpath, &size);
	if (ret != ERROR_SUCCESS || type != REG_SZ)
	    seedpath[0] = '\0';
	RegCloseKey(rkey);
    } else
