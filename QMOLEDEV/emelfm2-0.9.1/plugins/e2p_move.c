...
/**
@file plugins/e2p_mvbar.c
@brief plugin for moving selected items, with a progress-bar
*/
...
{
	VPATH *dlocal;	//includes localised destination path (maybe temp) of item moved
	guint64 done_size;
} E2_ProgressData;
...
static gboolean _e2p_mvbarQ (E2_ActionTaskData *qed);
...
*/
static E2_TwResult
_e2p_mvbar_twcb (VPATH *localpath, const struct stat *statptr,
...
This provides data for progressive updates during the course of moving an item.
...
*/
static void
_e2p_mvbar_progress (E2_ProgressData *data)
{
	gchar *localpath;
	struct stat sb;
#ifdef E2_VFS
	...
	//determine how much of current item has been moved
	E2_BarData pdata;
	pdata.totalsize = 0;
#ifdef E2_VFS
...
#endif
	{
		if (S_ISDIR (sb.st_mode))
		{
			...
			e2_fs_tw (localpath,
#endif
				_e2p_mvbar_twcb, &pdata, -1, E2TW_PHYS E2_ERR_NONE());
			//)
			//{
				//FIXME handle error
			//}
...
*/
static void
_e2p_mvbar_abort_clean_process (pid_t *pid)
{
	if (*pid != 0)
		kill (*pid, SIGSTOP);
...
/*static void
_e2p_mvbar_abort_clean_memory (gboolean **shared)
{
...
*/
static void
_e2p_mvbar_abort_clean_dialog (GtkWidget **dialog)
{
...
/**
@brief move item @a slocal to @a dlocal, while updating progress-window details
This is called with BGL open
@param slocal path of item to be moved, localised string
@param dlocal new path of moved item, localised string
@param realmove TRUE if this is a move between devices or partititions
...
_e2p_mvbar_exec (VPATH *slocal, VPATH *dlocal, gboolean realmove,
...
//	gchar *dest = F_FILENAME_FROM_LOCALE (VPSTR (dlocal));
	...
	//before move starts, get size of item to be moved
	E2_BarData pdata = { 0,0 };
	//if (!
		e2_fs_tw (slocal, _e2p_mvbar_twcb, &pdata, -1, E2TW_PHYS E2_ERR_NONE());
	//)
	//{
		//FIXME handle error
	//}
...
//	pthread_cleanup_push ((gpointer)_e2p_mvbar_abort_clean_memory, &results); FIXME
	...
	{	//child
		...
		results[0] = e2_task_backend_move (slocal,
#ifdef E2_VFS
			&tempdata);
#else
			templocal);
	...
	guint waitup;
	if (!realmove)
		waitup = 1000;  //shorter delay if the move is logical (same device)
	else
		waitup = 50000;  //longer delay if the move needs to be physical
	usleep (waitup);
	...
	{	//task not finished already, do progress reporting
		...
		pthread_cleanup_push ((gpointer)_e2p_mvbar_abort_clean_process, &pid);
...
*/
		...
		usleep (100000);	//small delay to (sort of) show the completion status ?
		//need to slow down ...
		//this approach is empirical, needs to be validated for various systems
//		if (realmove && bdata->count > 300 && bdata->count%10 == 0)
//		g_usleep (50000);	//small delay to allow filesystem to complete its business
//		if (!(wdata->flags & E2_BARTASK_PAUSED))
//			gtk_widget_hide (wdata->dialog);
...
*/
static void
_e2p_mvbar_response_cb (GtkDialog *dialog, gint response, E2_BarWindowData *wdata)
{
	NEEDCLOSEBGL
	switch (response)
	{
...
*/
static gboolean
_e2p_mvbar (gpointer from, E2_ActionRuntime *art)
{
	return (e2_task_enqueue_task (E2_TASK_MOVE, art, from,
		_e2p_mvbarQ, e2_task_refresh_lists));
}
static gboolean
_e2p_mvbarQ (E2_ActionTaskData *qed)
{
	...
	if (e2_fs_access (&sdata, W_OK E2_ERR_PTR()))
#else
	if (e2_fs_access (qed->currdir, W_OK E2_ERR_PTR()))
#endif
	{
		e2_fs_error_local (_("Cannot delete anything from %s"),
#ifdef E2_VFS
			&sdata E2_ERR_MSGL());
#else
			qed->currdir E2_ERR_MSGL());
#endif
		E2_ERR_CLEAR
		return FALSE;
	}
	...
	//determine whether move will be physical or logical
	gboolean realmove;
#ifdef E2_VFS
	if (qed->currspace != qed->othrspace)
		realmove = TRUE;
	else
	{
#endif
...
#else
		if (!e2_fs_stat (qed->currdir, &statbuf E2_ERR_NONE()))
#endif
...
#else
		if (!e2_fs_stat (qed->othrdir, &statbuf E2_ERR_NONE()))
#endif
			realmove = (statbuf.st_dev != device);
		else
			realmove = TRUE;
#ifdef E2_VFS
	}
#endif
	printd (DEBUG, (realmove) ? "physical move":"logical move");
...
//	hbox = e2_widget_add_box (dialog_vbox, FALSE, 0, TRUE, FALSE, 0);
//	windowdata.label2 = e2_widget_add_mid_label (hbox, "", 0.5, TRUE, 0);
	...
	E2_Button local_btn = { _("_Pause"), STOCK_NAME_MEDIA_PAUSE,
		//_("Suspend moving"), E2_BTN_TIPPED, E2_BTN_TIPPED,
		NULL, 0, 0,
		E2_RESPONSE_USER1 };
	...
	//local_btn.tip = _("Resume moving after pause");
	...
	for (count=0; count < names->len; count++, iterator++)
	{
		...
		//if (!
			...
			src->str,
#endif
			_e2p_mvbar_twcb, &totaldata, -1, E2TW_PHYS E2_ERR_NONE());
		//)
		//{
			//FIXME handle error
		//}
