...
/*
  ...
  - Mapping FOX keycodes to unicode and vice-versa.
  - X11 code comes from Markus G. Kuhn <mkuhn@acm.org>, and adapted to
    C++ and FOX by Jeroen van der Zijp.
*/
...
struct CodeMap { FXushort sym; FXushort ucs; };


static CodeMap sym2ucs[] = {
...
FXwchar fxkeysym2ucs(FXwchar sym){
  ...
  // Convert key pad to Latin-1
  if(0xffaa<=sym && sym<=0xffb9) return sym-0xff80;

  // Also check for directly encoded 24-bit UCS characters
  if((sym&0xff000000)==0x01000000) return sym&0x00ffffff;

  // Binary search in table
  do{
    m=(h+l)>>1;
    if(sym2ucs[m].sym==sym) return sym2ucs[m].ucs;
    if(sym2ucs[m].sym<sym) l=m+1; else h=m-1;
    }
  while(h>=l);
...
FXwchar fxucs2keysym(FXwchar ucs){
  ...
  do{
    m=(h+l)>>1;
    if(ucs2sym[m].ucs==ucs) return ucs2sym[m].sym;
    if(ucs2sym[m].ucs<ucs) l=m+1; else h=m-1;
    }
  while(h>=l);

  // By default return unicode key sym
