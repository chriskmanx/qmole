/*
 * LessTif ID : $Id: xdnd.c,v 1.1 2004/08/28 19:22:46 dannybackx Exp $
 *
 * This file is maintained by Paul Sheer.
 * Integration with LessTif should be outside this source, so Paul can
 * maintain this file and guarantee that other users (GTK+, ...) use
 * the same version.
 *
 * If the integration is not possible without changing this file,
 * please contact Paul.
 */
...
{
    XChangeProperty (dnd->display, window, dnd->XdndAware, XA_ATOM, 32, PropModeReplace, (unsigned char *) &dnd->version, 1);
    if (typelist) {
	int n;
	n = array_length (typelist);
	if (n)
	    XChangeProperty (dnd->display, window, dnd->XdndAware, XA_ATOM, 32, PropModeAppend, (unsigned char *) typelist, n);
    }
}
...
int xdnd_is_dnd_aware (DndClass * dnd, Window window, int *version, Atom * typelist)
{
    ...
    *version = 0;
    ...
    if (actual != XA_ATOM || format != 32 || count == 0 || !data) {
        DEBUGOUT(_LtDebug(__FILE__, NULL, "XGetWindowProperty failed in xdnd_is_dnd_aware - XdndAware = %ld", dnd->XdndAware));
    ...
    *version = dnd->version < types[0] ? dnd->version : types[0];	/* minimum */
    DEBUGOUT(_LtDebug(__FILE__, NULL,"Using XDND version %d", *version)); 
    if (count > 1) {
	result = 0;
	for (t = typelist; *t; t++) {
	    int j;
...
void xdnd_get_type_list (DndClass * dnd, Window window, Atom ** typelist)
{
    ...
    *typelist = 0;
	...
	if (data)
	    XFree (data);
        DEBUGOUT(_LtDebug(__FILE__, NULL, "XGetWindowProperty failed in xdnd_get_type_list - dnd->XdndTypeList = %ld", dnd->XdndTypeList));
	return;
    }
...
int xdnd_get_actions (DndClass * dnd, Window window, Atom ** actions, char ***descriptions)
{
	...
	*descriptions = malloc ((count + 1) * sizeof (char *));
	_XmWarning(NULL, "XGetWindowProperty no property or wrong format for action descriptions");
	for (i = 0; i < count; i++)
	    (*descriptions)[i] = "";
...
void xdnd_send_position (DndClass * dnd, Window window, Window from, Atom action, int x, int y, unsigned long time)
{
    XEvent xevent;
...
void xdnd_send_drop (DndClass * dnd, Window window, Window from, unsigned long time)
{
    XEvent xevent;
...
int xdnd_convert_selection (DndClass * dnd, Window window, Window requester, Atom type)
{
    if (window != XGetSelectionOwner (dnd->display, dnd->XdndSelection)) {
        DEBUGOUT(_LtDebug(__FILE__, NULL,"xdnd_convert_selection(): XGetSelectionOwner failed"));
	return 1;
    }
...
int xdnd_set_selection_owner (DndClass * dnd, Window window, Atom type)
{
    if (!XSetSelectionOwner (dnd->display, dnd->XdndSelection, window, CurrentTime)) {
        DEBUGOUT(_LtDebug(__FILE__, NULL, "xdnd_set_selection_owner(): XSetSelectionOwner failed"));
	return 1;
    }
    return 0;
}
...
{
    XEvent xevent;
    DEBUGOUT(_LtDebug(__FILE__, NULL, "      requestor = %ld", request->requestor));
    DEBUGOUT(_LtDebug(__FILE__, NULL, "      property = %ld", request->property));
    DEBUGOUT(_LtDebug(__FILE__, NULL, "      length = %d", length));
...
Atom xdnd_drag (DndClass * dnd, Window from, Atom action, Atom * typelist)
{
    ...
    if (!typelist)
	_XmWarning(NULL, "xdnd_drag() called with typelist = 0");

/* first wait until the mouse moves more than five pixels */
    do {
	XNextEvent (dnd->display, &xevent);
	if (xevent.type == ButtonRelease) {
            DEBUGOUT(_LtDebug(__FILE__, NULL, "button release - no motion"));
    ...
    DEBUGOUT(_LtDebug(__FILE__, NULL, "moved 5 pixels - going to drag"));
...
/* the mouse has been dragged a little, so this is a drag proper */
    if (XGrabPointer (dnd->display, dnd->root_window, False,
        ...
        DEBUGOUT(_LtDebug(__FILE__, NULL, "Unable to grab pointer"));
...
/* done, but must send a leave event */
            DEBUGOUT(_LtDebug(__FILE__, NULL, "ButtonRelease - exiting event loop"));
	    break;
	case MotionNotify:
...
/* last_window is just for debug purposes */
	    if (last_window != xevent.xmotion.subwindow) {
                DEBUGOUT(_LtDebug(__FILE__, NULL, "window crossing to %ld", xevent.xmotion.subwindow));
                DEBUGOUT(_LtDebug(__FILE__, NULL, "  current window is %ld", over_window));
	    }
	    dnd_aware = 0;
...
/* leaving window we were over */
		xevent.xany.window = over_window;
		if (over_window) {
		    if (dnd->stage == XDND_DRAG_STAGE_ENTERED) {
                        DEBUGOUT(_LtDebug(__FILE__, NULL, "got leave at right stage"));
			dnd->stage = XDND_DRAG_STAGE_DRAGGING;
			if (dnd->internal_drag) {
                            DEBUGOUT(_LtDebug(__FILE__, NULL, "  our own widget"));
			    if (dnd->widget_apply_leave)
				(*dnd->widget_apply_leave) (dnd, xevent.xany.window);
			} else {
                            DEBUGOUT(_LtDebug(__FILE__, NULL, "  not our widget - sending XdndLeave"));
			    xdnd_send_leave (dnd, xevent.xany.window, from);
			}
			dnd->internal_drag = 0;
			dnd->dropper_window = 0;
			dnd->ready_to_drop = 0;
		    } else {
			DEBUGOUT(_LtDebug(__FILE__, NULL, "got leave at wrong stage - ignoring"));
		    }
		}
/* entering window we are currently over */
		over_window = xevent.xmotion.subwindow;
		if (dnd_aware) {
		    DEBUGOUT(_LtDebug(__FILE__, NULL, "  is dnd aware"));
			...
			DEBUGOUT(_LtDebug(__FILE__, NULL, "    not our widget - sending XdndEnter"));
...
/* got here, so we are just moving `inside' the same window */
		if (dnd->stage == XDND_DRAG_STAGE_ENTERED) {
		    ...
		    DEBUGOUT(_LtDebug(__FILE__, NULL, "got motion at right stage"));
		    dnd->x = xevent.xmotion.x_root;
		    dnd->y = xevent.xmotion.y_root;
		    if (dnd->want_position && outside_rectangle (dnd->x, dnd->y, &dnd->rectangle)) {
			DEBUGOUT(_LtDebug(__FILE__, NULL, "  want position and outside rectangle"));
			if (dnd->internal_drag) {
			    DEBUGOUT(_LtDebug(__FILE__, NULL, "    our own widget"));
			    ...
			    DEBUGOUT(_LtDebug(__FILE__, NULL, "      return action=%ld", dnd->supported_action));
			} else {
			    DEBUGOUT(_LtDebug(__FILE__, NULL, "    not our own widget - sending XdndPosition"));
			    xdnd_send_position (dnd, over_window, from, action, dnd->x, dnd->y, xevent.xmotion.time);
			}
		    } else {
			DEBUGOUT(_LtDebug(__FILE__, NULL, "  doesn't want position or inside rectangle"));
			DEBUGOUT(_LtDebug(__FILE__, NULL, "    rectangle = (x=%d, y=%d, w=%d, h=%d), (x,y)=(%d,%d) want_position=%d\n", dnd->rectangle.x, dnd->rectangle.y, dnd->rectangle.width, dnd->rectangle.height, dnd->x, dnd->y, dnd->want_position));
		    }
		}
	    }
	    last_window = xevent.xmotion.subwindow;
	    break;
	case ClientMessage:
	    DEBUGOUT(_LtDebug(__FILE__, NULL, "ClientMessage recieved"));
	    if (xevent.xclient.message_type == dnd->XdndStatus && !dnd->internal_drag) {
		DEBUGOUT(_LtDebug(__FILE__, NULL, "  XdndStatus recieved"));
		if (dnd->stage == XDND_DRAG_STAGE_ENTERED && XDND_STATUS_TARGET_WIN (&xevent) == dnd->dropper_window) {
		    DEBUGOUT(_LtDebug(__FILE__, NULL, "    XdndStatus stage correct, dropper window correct"));
		    ...
		    DEBUGOUT(_LtDebug(__FILE__, NULL, "      rectangle = (x=%d, y=%d, w=%d, h=%d), want_position=%d\n", dnd->rectangle.x, dnd->rectangle.y, dnd->rectangle.width, dnd->rectangle.height, dnd->want_position));
		} else {
		    DEBUGOUT(_LtDebug(__FILE__, NULL, "    XdndStatus stage incorrect or dropper window incorrect"));
		}
	    }
	    break;
...
/* the target widget MAY request data, so wait for SelectionRequest */
		int length = 0;
		unsigned char *data = 0;
		DEBUGOUT(_LtDebug(__FILE__, NULL, "SelectionRequest - getting widget data"));

		(*dnd->widget_get_data) (dnd, from, &data, &length, xevent.xselectionrequest.target);
		if (data) {
		    DEBUGOUT(_LtDebug(__FILE__, NULL, "  sending selection"));
    ...
    if (dnd->ready_to_drop) {
	Time time;
	DEBUGOUT(_LtDebug(__FILE__, NULL, "ready_to_drop - sending XdndDrop"));
...
/* we are dealing with our own widget, no need to send drop events, just put the data straight */
		...
		if (data) {
			...
			DEBUGOUT(_LtDebug(__FILE__, NULL, "  inserted data into widget - success"));
		    } else {
			DEBUGOUT(_LtDebug(__FILE__, NULL, "  inserted data into widget - failed"));
		    }
		    xdnd_xfree (data);
		} else {
		    DEBUGOUT(_LtDebug(__FILE__, NULL, "  got data from widget, but data is null"));
		}
	    }
	} else {
	    xdnd_set_selection_owner (dnd, from, dnd->desired_type);
	...
	if (!dnd->internal_drag)
		...
		if (xevent.type == ClientMessage && xevent.xclient.message_type == dnd->XdndFinished) {
		    DEBUGOUT(_LtDebug(__FILE__, NULL, "XdndFinished"));
		    if (XDND_FINISHED_TARGET_WIN (&xevent) == dnd->dropper_window) {
			DEBUGOUT(_LtDebug(__FILE__, NULL, "  source correct - exiting event loop, action=%ld", dnd->supported_action));
			result = dnd->supported_action;		/* success - so return action to caller */
			break;
		    }
			...
			DEBUGOUT(_LtDebug(__FILE__, NULL, "timeout - exiting event loop"));
			break;
		    }
...
/* don't wait for a XdndFinished event */
	...
	DEBUGOUT(_LtDebug(__FILE__, NULL, "not ready_to_drop - ungrabbing pointer"));
    }
...
int xdnd_handle_drop_events (DndClass * dnd, XEvent * xevent)
{
	...
	DEBUGOUT(_LtDebug(__FILE__, NULL, "got SelectionNotify"));
	if (xevent->xselection.property == dnd->Xdnd_NON_PROTOCOL_ATOM && dnd->stage == XDND_DROP_STAGE_CONVERTING) {
	    int error;
	    DEBUGOUT(_LtDebug(__FILE__, NULL, "  property is Xdnd_NON_PROTOCOL_ATOM - getting selection"));
...
/* error is not actually used, i think future versions of the protocol maybe should return 
   an error status to the calling window with the XdndFinished client message */
	...
	if (xevent->xclient.message_type == dnd->XdndEnter) {
		...
		DEBUGOUT(_LtDebug(__FILE__, NULL, "    wrong stage or from wrong window"));
	    }
	} else if (xevent->xclient.message_type == dnd->XdndPosition) {
	    DEBUGOUT(_LtDebug(__FILE__, NULL, "  message_type is XdndPosition"));
		...
		DEBUGOUT(_LtDebug(__FILE__, NULL, "    will accept = %d", dnd->will_accept));
		DEBUGOUT(_LtDebug(__FILE__, NULL, "    sending status"));
		DEBUGOUT(_LtDebug(__FILE__, NULL, "      action=%ld, rectangle = (x=%d, y=%d, w=%d, h=%d), (x,y)=(%d,%d) want_position=%d\n", dnd->supported_action, rectangle.x, rectangle.y, rectangle.width, rectangle.height, dnd->x, dnd->y, want_position));
	    ...
	    DEBUGOUT(_LtDebug(__FILE__, NULL, "  message_type is XdndDrop"));
	    if (dnd->dragger_window == XDND_DROP_SOURCE_WIN (xevent) && dnd->stage == XDND_DROP_STAGE_ENTERED) {
		dnd->time = CurrentTime;
		if (dnd_version_at_least (dnd->dragging_version, 1))
		    dnd->time = XDND_DROP_TIME (xevent);
		if (dnd->will_accept) {
		    DEBUGOUT(_LtDebug(__FILE__, NULL, "    will_accept is true - converting selectiong"));
		    DEBUGOUT(_LtDebug(__FILE__, NULL, "      my window is %ld", dnd->dropper_window));
		    DEBUGOUT(_LtDebug(__FILE__, NULL, "        source window is %ld", dnd->dragger_window));
		    xdnd_convert_selection (dnd, dnd->dragger_window, dnd->dropper_window, dnd->desired_type);
		    dnd->stage = XDND_DROP_STAGE_CONVERTING;
		} else {
		    DEBUGOUT(_LtDebug(__FILE__, NULL, "    will_accept is false - sending finished"));
		    if (dnd_version_at_least (dnd->dragging_version, 1))
