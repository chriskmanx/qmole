/* $Header: /cvsroot/lesstif/lesstif/test/Xm/scrolledwindow/test9.c,v 1.4 2001/05/23 14:30:30 amai Exp $
	...
	XtPointer       call_data;
{
	...
	if (canvas->y_offset == new_value)
		return;
	XtVaGetValues(canvas->vsb, XmNsliderSize, &slider_size, NULL);
	/* Work out how far (and in which direction) we need to scroll */
	if ((positive = (new_value > canvas->y_offset)))
		pixels_to_lose = new_value - canvas->y_offset;
	else
		pixels_to_lose = canvas->y_offset - new_value;
	canvas->y_offset = new_value;
	if (pixels_to_lose >= slider_size)
		/* Scrolling more than a page, nothing worth copying */
		XClearArea(display, window, 0, 0, 0, 0, True);
	else {
  ...
  
{
   ...
   CWWidth | CWHeight            ,  508,  524,  200,  200, 0,0,0, /* form */
