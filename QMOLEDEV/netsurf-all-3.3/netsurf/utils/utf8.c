	     ...
	     size_t *result_len)
{
	...
	/* perform conversion */
	if (libiconv(cd, (void *) &in, &slen, &out, &rlen) == (size_t)-1) {
		free(temp);
			...
			size_t *outlen)
{
	...
	while (inlen > 0) {
		ret = libiconv(cd, (void *) &chunk, &inlen, (void *) out, outlen);
		if (ret != (size_t) -1)
			break;
		...
		pescape = escape;
		ret = libiconv(cd, (void *) &pescape, &esclen,
				(void *) out, outlen);
		if (ret == (size_t) -1)
			return NSERROR_NOMEM;
...
utf8_to_html(const char *string, const char *encname, size_t len, char **result)
{
	...
	if (strncasecmp(last_cd.from, "UTF-8", sizeof(last_cd.from)) == 0 &&
		...
		/* no match, so create a new cd */
		cd = libiconv_open(encname, "UTF-8");
		if (cd == (iconv_t) -1) {
			if (errno == EINVAL)
				return NSERROR_BAD_ENCODING;
			/* default to no memory */
