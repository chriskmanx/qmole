...
let push_callback () =
  (!enter_callback ();
   let old = !stop_emit_ref in (stop_emit_ref := false; State old))
  
let pop_callback =
  function
  | State old ->
      let res = !stop_emit_ref
      in (stop_emit_ref := old; !exit_callback (); res)
...
let connect ~sgn: ((sgn : ('a, _) t)) ~callback ?(after = false)
            (obj : 'a obj) =
  let callback argv =
    ...
    in
      (safe_call (sgn.marshaller callback) argv
       ...
       if pop_callback old then emit_stop_by_name obj ~name: sgn.name else ())
  in
    connect_by_name obj ~name: sgn.name ~callback: (Closure.create callback)
      ~after
  
external handler_block : 'a obj -> id -> unit = "ml_g_signal_handler_block"
  
external handler_unblock : 'a obj -> id -> unit =
  "ml_g_signal_handler_unblock"
  
external disconnect : 'a obj -> id -> unit = "ml_g_signal_handler_disconnect"
...
let marshal4 conv1 conv2 conv3 conv4 name f argv =
  ...
  let (arg1, arg2, arg3, arg4) =
    try ((get conv1 1), (get conv2 2), (get conv3 3), (get conv4 4))
...
let marshal6 conv1 conv2 conv3 conv4 conv5 conv6 name f argv =
  ...
  let (arg1, arg2, arg3, arg4, arg5, arg6) =
    try
      ((get conv1 1), (get conv2 2), (get conv3 3), (get conv4 4),
       (get conv5 5), (get conv6 6))
    with | _ -> failwith ("GtkSignal.marhsal6 : " ^ name)
  in f arg1 arg2 arg3 arg4 arg5 arg6
  
let set_result conv argv res = Closure.set_result argv (conv.inj res)
  
let marshal0_ret ~ret f argv = set_result ret argv (f ())
...
let emit (obj : 'a obj) ~sgn: ((sgn : ('a, 'b) t))
         ~emitter: ((emitter : cont: ((_ data_set) array -> 'c) -> 'b))
         ~conv: ((conv : g_value -> 'c)) =
  emitter
    ~cont: (fun params -> conv (emit_by_name obj ~name: sgn.name ~params))
...
external _override_class_closure : string -> g_type -> g_closure -> unit =
  "ml_g_signal_override_class_closure"
  
let override_class_closure { name = name } t c =
  _override_class_closure name t c
  
external chain_from_overridden : Closure.argv -> unit =
