...
let error_loc = Location.fresh ()
let error_patt = { (mkpat Ppat_any) with ppat_loc = error_loc }
let error_expr = { (mkexp Pexp_assertfalse) with pexp_loc = error_loc }

let mkint = function
  | InRange i  -> Const_int i
  | Overflow _ -> Const_int max_int
let mkint32 = function
  | InRange i  -> Const_int32 i
  | Overflow _ -> Const_int32 Int32.max_int
let mkint64 = function
  | InRange i  -> Const_int64 i
  | Overflow _ -> Const_int64 Int64.max_int
let mkchar = function
  | InRange i  -> Const_char i
  | Overflow _ -> Const_char 'x'
let mknativeint = function
  | InRange i  -> Const_nativeint i
  | Overflow _ -> Const_nativeint Nativeint.max_int
let mkminusint = function
  | InRange i  -> Const_int (-i)
  | Overflow _ -> Const_int max_int
let mkminusint32 = function
  | InRange i  -> Const_int32 (Int32.neg i)
  | Overflow _ -> Const_int32 Int32.min_int
let mkminusint64 = function
  | InRange i  -> Const_int64 (Int64.neg i)
  | Overflow _ -> Const_int64 Int64.min_int
let mkminusnativeint = function
  | InRange i  -> Const_nativeint (Nativeint.neg i)
  | Overflow _ -> Const_nativeint Nativeint.min_int
let mkdirint = function
  | InRange i  -> Pdir_int i
  | Overflow s -> Pdir_string s
...
let mkoperator name pos =
  ...
  { pexp_desc = Pexp_ident(longident loc (Lident (name, loc)));
    pexp_loc = rhs_loc pos }

let lident i =
  let loc = symbol_rloc () in
  longident loc (Lident (i, loc))
let ldot lid i =
  let loc = symbol_rloc () in
  remove_longident lid;
  longident loc (Ldot (lid, i, loc))
let lapply lid lid' =
  let loc = symbol_rloc () in
  remove_longident lid;
  remove_longident lid';
  longident loc (Lapply (lid, lid', loc))
...
let mkassert e =
  match e with
  | {pexp_desc = Pexp_construct (Lident ("false", _), None, false) } ->
...
let rec mktailexp = function
    [] ->
      ghexp(Pexp_construct(Lident ("[]", symbol_gloc ()), None, false))
  | e1 :: el ->
      ...
      {pexp_desc = Pexp_construct(Lident ("::", symbol_gloc ()), Some arg, false); pexp_loc = l}

let rec mktailpat = function
    [] ->
      ghpat(Ppat_construct(Lident ("[]", symbol_gloc ()), None, false))
  | p1 :: pl ->
...
let rec mkrangepat c1 c2 =
  if c1 > c2 then
    ...
    mkpat(Ppat_constant(Const_char c1))
  else
    reloc_pat (deep_mkrangepat c1 c2)


let mkrangepat c1 c2 =
  match c1, c2 with
    | InRange c1, InRange c2 -> mkrangepat c1 c2
    | _                      -> error_patt
...
let bigarray_function str name =
  Ldot(Ldot(Lident ("Bigarray", Location.none), str, Location.none),
       name,
       Location.none)
...
let exp_of_label lbl =
  mkexp (Pexp_ident(Lident(Longident.last lbl, Longident.location lbl)))
...
class_type:
  ...
  | QUESTION LIDENT COLON simple_core_type_or_tuple MINUSGREATER class_type
      { mkcty(Pcty_fun("?" ^ $2 ,
                       {ptyp_desc =
                        Ptyp_constr
			  (Ldot (Lident ("*predef*", Location.none),
				 "option", $4.ptyp_loc),
			   [$4]);
                        ptyp_loc = $4.ptyp_loc},
                       $6)) }
  | OPTLABEL simple_core_type_or_tuple MINUSGREATER class_type
      { mkcty(Pcty_fun("?" ^ $1 ,
                       {ptyp_desc =
                        Ptyp_constr(Ldot (Lident ("*predef*", Location.none),
					  "option", $2.ptyp_loc),
				    [$2]);
                        ptyp_loc = $2.ptyp_loc},
                       $4)) }
...
simple_expr:
  ...
  | simple_expr SHARP label
      { mkexp(Pexp_send($1, {id = $3 ; loc = rhs_loc 3})) }
...
let_binding:
    val_ident fun_binding
      { ({ppat_desc = Ppat_var $1.id; ppat_loc = rhs_loc 1}, $2) }
  | val_ident COLON typevar_list DOT core_type EQUAL seq_expr
      { (ghpat(Ppat_constraint({ppat_desc = Ppat_var $1.id; ppat_loc = rhs_loc 1},
                               ghtyp(Ptyp_poly($3,$5)))),
         $7) }
...
;
type_parameter:
    type_variance QUOTE ident                   { $3.id, $1 }
;
type_variance:
    /* empty */                                 { false, false }
  | PLUS                                        { true, false }
...
label_declaration:
    ...
    { ({id = $2 ; loc = rhs_loc 2}, $1, $4, symbol_rloc()) }
;
...
typevar_list:
        QUOTE ident                             { [$2.id] }
      | typevar_list QUOTE ident                { $3.id :: $1 }
;
poly_type:
        core_type
          { mktyp(Ptyp_poly([], $1)) }
...
core_type2:
  ...
  | QUESTION LIDENT COLON core_type2 MINUSGREATER core_type2
      { mktyp(Ptyp_arrow("?" ^ $2 ,
               {ptyp_desc = Ptyp_constr(Ldot (Lident ("*predef*", Location.none),
					      "option", $4.ptyp_loc),
					[$4]);
...
constant:
  ...
  | INT32                                       { mkint32 $1 }
  | INT64                                       { mkint64 $1 }
  | NATIVEINT                                   { mknativeint $1 }
;
signed_constant:
    constant                                    { $1 }
  | MINUS INT                                   { mkminusint $2 }
  | MINUS FLOAT                                 { Const_float("-" ^ $2) }
  | MINUS INT32                                 { mkminusint32 $2 }
  | MINUS INT64                                 { mkminusint64 $2 }
  | MINUS NATIVEINT                             { mkminusnativeint $2 }
  | PLUS INT                                    { mkint $2 }
  | PLUS FLOAT                                  { Const_float $2 }
  | PLUS INT32                                  { mkint32 $2 }
  | PLUS INT64                                  { mkint64 $2 }
  | PLUS NATIVEINT                              { mknativeint $2 }
;

/* Identifiers and long identifiers */

uident:
    UIDENT { {id = $1 ; loc = symbol_rloc ()} }
lident:
    LIDENT { {id = $1 ; loc = symbol_rloc ()} }
...
val_ident:
    lident                                      { $1 }
  | LPAREN operator RPAREN                      { {id = $2 ; loc = rhs_loc 2} }
;
operator:
    PREFIXOP                                    { $1 }
  | INFIXOP0                                    { $1 }
...
;
constr_ident:
    constr_ident_no_loc                         { {id = $1 ; loc = symbol_rloc ()} }

val_longident:
    val_ident                                   { lident $1.id }
  | mod_longident DOT val_ident                 { ldot $1 $3.id }
;
constr_longident:
...
mty_longident:
    ident                                       { lident $1.id }
  | mod_ext_longident DOT ident                 { ldot $1 $3.id }
;
clty_longident:
    LIDENT                                      { lident $1 }
  | mod_ext_longident DOT LIDENT                { ldot $1 $3 }
...
toplevel_directive:
    SHARP ident                 { Ptop_dir($2.id, Pdir_none) }
  | SHARP ident STRING          { Ptop_dir($2.id, Pdir_string $3) }
  | SHARP ident INT             { Ptop_dir($2.id, mkdirint $3) }
  | SHARP ident val_longident   { Ptop_dir($2.id, Pdir_ident $3) }
  | SHARP ident FALSE           { Ptop_dir($2.id, Pdir_bool false) }
  | SHARP ident TRUE            { Ptop_dir($2.id, Pdir_bool true) }
;

/* Miscellaneous */

name_tag:
    BACKQUOTE ident                             { $2.id }
;
rec_flag:
    /* empty */                                 { Nonrecursive }
  | REC                                         { Recursive }
