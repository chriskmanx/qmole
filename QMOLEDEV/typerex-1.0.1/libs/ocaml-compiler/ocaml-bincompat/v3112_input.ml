    ...
    match p with
      ...
      | T.Ppat_record list ->
        Ppat_record (List.map (fun (l, p) ->
          (mknoloc l, pattern p)) list,  Closed)
...
module TYPES : sig
    ...
    val signature :
      V3112_types.Types.signature_item list -> Types.signature_item list
...
let input_ast_intf ic =
  ...
  let intf = (input_value ic : V3112_types.Parsetree.signature) in
  let intf = PARSETREE.signature intf in
  (input_name, intf)

let input_ast_impl ic =
  let input_name = (input_value ic : string) in
  let impl = (input_value ic : V3112_types.Parsetree.structure) in
...
module TYPES : sig
    ...
    and type_desc desc =
      ...
      | T.Ttuple list -> Ttuple (List.map type_expr list)
      | T.Tconstr (p, list, ab) ->
          Tconstr (p, List.map type_expr list, ref (abbrev_memo !ab))
      | T.Tobject (t, { contents = None }) ->
          Tobject (type_expr t, ref None)
      | T.Tobject (t, { contents = Some (p, list) }) ->
          Tobject (type_expr t, ref (Some (p, List.map type_expr list)))
      | T.Tfield (s, f, t1, t2) ->
          Tfield (s, field_kind f, type_expr t1, type_expr t2)
      | T.Tnil -> Tnil
      | T.Tlink t -> Tlink (type_expr t)
    ...
    and abbrev_memo ab =
      match ab with
        T.Mnil -> Mnil
      | T.Mcons (pf, p, t1, t2, ab) ->
          Mcons(pf, p, type_expr t1, type_expr t2,
            abbrev_memo ab)
      | T.Mlink ab -> Mlink (ref (abbrev_memo !ab))
    ...
    and row_desc r =
      { row_fields = List.map (fun (l, rf) ->
        ...
        row_name = (match r.T.row_name with
            None -> None
          | Some (p, list) ->
              Some ( p, List.map type_expr list));
      }
    ...
    and signature_item item =
      match item with
        T.Tsig_value (id, v) ->
          Sig_value (id, value_description v)
      | T.Tsig_type (id, t, r) ->
          Sig_type (id, type_declaration t, rec_status r)
      | T.Tsig_exception (id, decl) ->
          Sig_exception (id, exception_declaration decl)
      | T.Tsig_module (id, m, r) ->
          Sig_module (id,  module_type m, rec_status r)
      | T.Tsig_modtype (id, m) ->
          Sig_modtype (id, modtype_declaration m)
      | T.Tsig_class (id, cl, r) ->
          Sig_class (id, class_declaration cl, rec_status r)
      | T.Tsig_cltype (id, cl, r) ->
        Sig_class_type (id, cltype_declaration cl, rec_status r)
    ...
    and value_kind v =
      ...
      | T.Val_self (meths2, vars2, s, t) ->
          ...
          let vars = ref Vars.empty in
          T.Meths.iter (fun s (id,t) ->
              meths := Meths.add s (id, type_expr t) !meths) !meths2;
          T.Vars.iter (fun s (id, mf, vf, t) ->
              vars := Vars.add s (id,
                mf, vf, type_expr t) !vars) !vars2;
          Val_self(meths, vars, s, type_expr t)
      | T.Val_anc (list, s) ->
          Val_anc (List.map (fun (s,id) -> (s, id)) list, s)
          | T.Val_unbound -> Val_unbound
    ...
    and type_kind t =
      match t with
        T.Type_abstract -> Type_abstract
      | T.Type_variant list -> Type_variant (List.map (fun (s, tlist) ->
                (Ident.create s, List.map type_expr tlist, None)) list)
      | T.Type_record (list, rr) ->
          Type_record (List.map (fun (s, mf, t) ->
                (s, mf, type_expr t)) list,
            record_representation rr)
    ...
    and module_type decl =
      match decl with
        T.Tmty_ident p -> Tmty_ident (p)
      | T.Tmty_signature s ->
          Tmty_signature (signature s)
      | T.Tmty_functor (id, m1, m2) ->
          Tmty_functor (id, module_type m1, module_type m2)
    ...
    and class_type cl =
      match cl with
        T.Tcty_constr (p, list, ct) ->
          Tcty_constr (p, List.map type_expr list, class_type ct)
      | T.Tcty_signature cl ->
    ...
    and class_signature c =
      ...
      { cty_self = type_expr c.T.cty_self;
        ...
        cty_concr = !cty_concr;
        cty_inher = List.map (fun (p, l) ->
            (p, List.map type_expr l)) c.T.cty_inher;
      }

  and cltype_declaration c =
      { clty_params = List.map type_expr c.T.clty_params;
        clty_type = class_type c.T.clty_type;
        clty_path = c.T.clty_path;
        clty_variance = c.T.clty_variance;
      }
...
let signature sg =
  TYPES.reset ();
  List.map TYPES.signature_item sg
