...
let input_cmx_file ic magic =
  if magic = V3112_types.cmx_magic_number then
    let ui = (input_value ic : V3112_types.Cmx_format.unit_infos) in
    CMX.Cmx_format.unit_infos ui
  else
    raise (Error (No_Such_Magic ("cmx", magic)))

let input_cmxa_file ic magic =
  if magic = V3112_types.cmxa_magic_number then
    let infos = (input_value ic : V3112_types.Cmx_format.library_infos) in
    CMX.Cmx_format.library_infos infos
  else
    raise (Error (No_Such_Magic ("cmxa", magic)))
