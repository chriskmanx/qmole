...
module ASTTYPES = struct
  let private_flag f = f
  let mutable_flag f = f
  let virtual_flag f = f
end
module IDENT = struct let t ident = ident end
module PATH = struct let t path = path end
module PRIMITIVE = struct let description prim = prim end
(*
 : sig
  ...
  let rec t p =
    match p with
        Pident id -> T.Pident (IDENT.t id)
      | Pdot(p, s, pos) -> T.Pdot (t p, s, pos)
      | Papply (p1, p2) -> T.Papply (t p1, t p2)
...
module TYPES : sig
    ...
    and type_desc desc =
      ...
      | Tpackage _ ->
	raise (Error (No_Such_Feature (this_version, "package type")))
    ...
    and type_kind t =
      ...
      | Type_variant list -> T.Type_variant (List.map (fun (s, tlist, gadt) ->
        match gadt with
            None -> (Ident.name s, List.map type_expr tlist)
          | Some gadt ->
