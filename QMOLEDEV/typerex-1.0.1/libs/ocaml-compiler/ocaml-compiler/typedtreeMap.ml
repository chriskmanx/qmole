...
module MakeMap(Map : MapArgument) : sig
    ...
    (*      Map.enter_binding pat exp;
	    let pat = map_pattern pat in
	    map_expression exp;
	    Map.leave_binding pat exp
    *)

    and map_bindings rec_flag list =
      (*      Map.enter_bindings rec_flag; *)
      List.map map_binding list;
    (*      Map.leave_bindings rec_flag *)
    ...
    and map_structure_item item =
      ...
      let str_desc =
        match item.str_desc with
          ...
          | Tstr_primitive (id, name, v) -> Tstr_primitive (id, name, map_value_description v)
          | Tstr_type list -> Tstr_type (List.map (fun (id, name, decl) -> (id, name, map_type_declaration decl) ) list)
          | Tstr_exception (id, name, decl) -> Tstr_exception (id, name, map_exception_declaration decl)
          | Tstr_exn_rebind (id, name, path, lid) -> Tstr_exn_rebind (id, name, path, lid)
          | Tstr_module (id, name, mexpr) -> Tstr_module (id, name, map_module_expr mexpr)
          | Tstr_recmodule list ->
	    let list =
	    ...
	    in
	    Tstr_recmodule list
          | Tstr_modtype (id, name, mtype) -> Tstr_modtype (id, name, map_module_type mtype)
        ...
        match pat.pat_desc with
          | Tpat_alias (pat1, kind) ->
            ...
            let kind =
              match kind with
                  TPat_alias _ -> kind
		| TPat_type (path, lid) -> kind
		| TPat_constraint ct -> TPat_constraint (map_core_type ct)
            in
	    Tpat_alias (pat1, kind)
	  | Tpat_tuple list -> Tpat_tuple (List.map map_pattern list)
          | Tpat_construct (path, lid, cstr_decl, args) -> Tpat_construct (path, lid, cstr_decl, List.map map_pattern args)
	    ...
	    Tpat_variant (label, pato, rowo)
          | Tpat_record (list, closed) ->
            Tpat_record (List.map (fun (path, lid, lab_desc, pat) -> (path, lid, lab_desc, map_pattern pat) ) list, closed)
    ...
    and map_expression exp =
      ...
      let exp_desc =
	    ...
	    Texp_let (rec_flag,
          ...
          | Texp_construct (path, lid, cstr_desc, args) ->
            Texp_construct (path, lid, cstr_desc,  List.map map_expression args )
	  ...
	  | Texp_open (path, lid, exp) -> Texp_open (path, lid, map_expression exp)
	  | Texp_newtype (s, exp) ->
    ...
    and map_signature_item item =
      ...
      let sig_desc =
        match item.sig_desc with
            Tsig_value (id, name, v) -> Tsig_value (id, name, map_value_description v)
          ...
          | Tsig_recmodule list ->
            Tsig_recmodule (List.map (fun (id, name, mtype) -> (id, name, map_module_type mtype) ) list)
    ...
    and map_modtype_declaration mdecl =
      ...
      let mdecl =
        match mdecl with
          ...
          | Tmodtype_manifest mtype -> Tmodtype_manifest (map_module_type mtype)
      in
    ...
    and map_module_type mty =
      ...
      let mty_desc =
	    ...
	    Tmty_functor (id, name, map_module_type mtype1, map_module_type mtype2)
      ...
      let mod_desc =
        match mexpr.mod_desc with
          ...
          | Tmod_functor (id, name, mtype, mexpr) ->
            Tmod_functor (id, name, map_module_type mtype, map_module_expr mexpr)
          ...
          | Tmod_constraint (mexpr, mod_type, Tmodtype_implicit, coercion ) ->
            Tmod_constraint (map_module_expr mexpr, mod_type, Tmodtype_implicit, coercion)
          | Tmod_constraint (mexpr, mod_type, Tmodtype_explicit mtype, coercion) ->
            Tmod_constraint (map_module_expr mexpr, mod_type, Tmodtype_explicit (map_module_type mtype), coercion)
      ...
      let cl_desc =
        match cexpr.cl_desc with
          | Tcl_constraint (cl, None, string_list1, string_list2, concr ) ->
            Tcl_constraint (map_class_expr cl, None, string_list1, string_list2, concr)
          | Tcl_structure clstr -> Tcl_structure (map_class_structure clstr)
          | Tcl_fun (label, pat, priv, cl, partial) ->
            Tcl_fun (label, map_pattern pat,
		     List.map (fun (id, name, exp) -> (id, name, map_expression exp)) priv,
		     map_class_expr cl, partial)

          | Tcl_apply (cl, args) ->
            Tcl_apply (map_class_expr cl,
		       List.map (fun (label, expo, optional) -> (label, may_map map_expression expo, optional)
		       ) args)
          | Tcl_let (rec_flat, bindings, ivars, cl) ->
            Tcl_let (rec_flat, map_bindings rec_flat bindings,
		     List.map (fun (id, name, exp) -> (id, name, map_expression exp)) ivars,
		     map_class_expr cl)

          | Tcl_constraint (cl, Some clty, vals, meths, concrs) ->
            Tcl_constraint ( map_class_expr cl, Some (map_class_type clty), vals, meths, concrs)
    ...
    and map_core_type ct =
      ...
      let ctyp_desc =
        match ct.ctyp_desc with
          ...
          | Ttyp_constr (path, lid, list) -> Ttyp_constr (path, lid, List.map map_core_type list)
          | Ttyp_object list -> Ttyp_object (List.map map_core_field_type list)
          | Ttyp_class (path, lid, list, labels) -> Ttyp_class (path, lid, List.map map_core_type list, labels)
          | Ttyp_alias (ct, s) -> Ttyp_alias (map_core_type ct, s)
          | Ttyp_variant (list, bool, labels) -> Ttyp_variant (List.map map_row_field list, bool, labels)
    ...
    and map_class_field cf =
	  ...
	  | Tcf_val (lab, name, mut, ident, Tcfk_virtual cty, override) ->
            Tcf_val (lab, name, mut, ident, Tcfk_virtual (map_core_type cty), override)
	  | Tcf_val (lab, name, mut, ident, Tcfk_concrete exp, override) ->
            Tcf_val (lab, name, mut, ident, Tcfk_concrete (map_expression exp), override)
	  | Tcf_meth (lab, name, priv, Tcfk_virtual cty, override) ->
            Tcf_meth (lab, name, priv, Tcfk_virtual (map_core_type cty), override)
	  | Tcf_meth (lab, name, priv, Tcfk_concrete exp, override) ->
            Tcf_meth (lab, name, priv, Tcfk_concrete (map_expression exp), override)
	  | Tcf_let (rec_flag, bindings, exps) ->
            Tcf_let (rec_flag, map_bindings rec_flag bindings,
