...
*)
    ...
    *)
    ...
    let analyse_class_structure env current_class_name tt_class_sig last_pos pos_limit p_cls tt_cls table =
      ...
      | ((Parsetree.Pcf_val (label, mutable_flag, _, _) |
          Parsetree.Pcf_valvirt (label, mutable_flag, _) ) as x) ->
            let virt = match x with Parsetree.Pcf_val _ -> false | _ -> true in
            let complete_name = Name.concat current_class_name label.txt in
            ...
            try
              if virt then
                Typedtree_search.search_virtual_attribute_type table
                  (Name.simple current_class_name) label.txt
              else
                Typedtree_search.search_attribute_type tt_cls label.txt
            with Not_found ->
        ...
        | (Parsetree.Pcf_virt  (label, private_flag, _)) ->
            let complete_name = Name.concat current_class_name label.txt in
            let (info_opt, ele_comments) = get_comments_in_class last_pos loc.Location.loc_start.Lexing.pos_cnum in
            let met_type =
              try Odoc_sig.Signature_search.search_method_type label.txt tt_class_sig
              with Not_found -> raise (Failure (Odoc_messages.method_type_not_found current_class_name label.txt))
            in
            let real_type =
        ...
        | (Parsetree.Pcf_meth  (label, private_flag, _, _)) ->
            let complete_name = Name.concat current_class_name label.txt in
            let (info_opt, ele_comments) = get_comments_in_class last_pos loc.Location.loc_start.Lexing.pos_cnum in
            let exp =
              try Typedtree_search.search_method_expression tt_cls label.txt
   ...
   and analyse_structure_item env current_module_name loc pos_limit comment_opt parsetree_item_desc typedtree
      ...
      | Parsetree.Pstr_primitive (name_pre, val_desc) ->
          ...
          print_DEBUG ("Parsetree.Pstr_primitive ("^name_pre.txt^", ["^(String.concat ", " val_desc.Parsetree.pval_prim)^"]");
          let typ = Typedtree_search.search_primitive table name_pre.txt in
      ...
      | Parsetree.Pstr_type name_typedecl_list ->
          ...
          let rec f ?(first=false) maybe_more_acc last_pos name_type_decl_list =
            ...
            | (name, type_decl) :: q ->
                ...
                in
                let tt_type_decl =
                  try Typedtree_search.search_type_declaration table name.txt
          ...
          let new_ex =
            {
              ...
              ex_info = comment_opt ;
                ex_args = List.map (fun ctyp ->
                    Odoc_env.subst_type new_env ctyp.ctyp_type) tt_excep_decl ;
              ex_alias = None ;
      ...
      | Parsetree.Pstr_modtype (name, modtype) ->
          ...
          let new_env2 =
            match tt_module_type.mty_type with
              (* A VOIR : cela peut-il être Mty_ident ? dans ce cas, on n'aurait pas la signature *)
              Types.Mty_signature s ->
