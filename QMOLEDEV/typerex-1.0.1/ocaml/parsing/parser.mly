...
let_binding:
    val_ident fun_binding
      { ({ppat_desc = Ppat_var (mkrhs $1 1); ppat_loc = rhs_loc 1}, $2) }
  | val_ident COLON typevar_list DOT core_type EQUAL seq_expr
      { (ghpat(Ppat_constraint({ppat_desc = Ppat_var (mkrhs $1 1); ppat_loc = rhs_loc 1},
                               ghtyp(Ptyp_poly($3,$5)))),
         $7) }
...
;
package_type_cstr:
    TYPE LIDENT EQUAL core_type { (mkrhs $2 2, $4) }
;
package_type_cstrs:
    package_type_cstr { [$1] }
