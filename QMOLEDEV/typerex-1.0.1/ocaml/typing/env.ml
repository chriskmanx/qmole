...
module EnvLazy : sig
  ...
  let create maker args =
    ...
    if !eager then (try ignore (force x) with _ ->  ());
    x
...
let diff env1 env2 =
  ...
  diff_keys is_local (*_exn*) env1.constrs env2.constrs @
...
let components_of_module' =
  ref ((fun ?norename env sub path mty -> assert false) :
          ?norename:bool -> t -> Subst.t -> Path.t -> module_type ->
          module_components)
...
let read_cmi_fun = ref read_cmi

let read_pers_struct modname filename =
  try
    let cmi = !read_cmi_fun filename in
    let comps =
      !components_of_module' ~norename:true empty Subst.identity
                             (Pident(Ident.create_persistent cmi.cmi_name))
                             (Mty_signature cmi.cmi_sign) in
...
let read_pers_struct_fun = ref read_pers_struct
let read_pers_struct modname filename =
  !read_pers_struct_fun modname filename

let rec find_cmi name cmi_exts =
  match cmi_exts with
      [] -> raise Not_found
    | cmi_ext :: tail ->
      try
        find_in_path_uncap !load_path (name ^ cmi_ext)
      with Not_found ->
        find_cmi name tail
...
let find_pers_struct name =
  ...
  with Not_found ->
    read_pers_struct name (find_cmi name !Clflags.cmi_exts)

let find_pers_struct_fun = ref find_pers_struct
let find_pers_struct name = !find_pers_struct_fun name
...
let rec components_of_module ?norename env sub path mty =
  EnvLazy.create lazy_components_of_module (norename, env, sub, path, mty)

and components_of_module_maker (norename, env, sub, path, mty) =
  (if norename = Some true then Ident.disabling_renaming else function x -> x)
    components_of_module_maker0 (norename, env, sub, path, mty)

and components_of_module_maker0 (norename, env, sub, path, mty) =
    ...
    Mty_signature sg ->
      ...
      List.iter2 (fun item path ->
        ...
        | Sig_module(id, mty, _) ->
            ...
            let comps = components_of_module ?norename !env sub path mty in
            c.comp_components <-
              Tbl.add (Ident.name id) (comps, !pos) c.comp_components;
            env := store_module ?norename id path mty !env;
            incr pos
        | Sig_modtype(id, decl) ->
...
and store_module ?norename id path mty env =
  { env with
    modules = Ident.add id (path, mty) env.modules;
    components =
      Ident.add id (path, components_of_module ?norename env Subst.identity path mty)
                   env.components;
