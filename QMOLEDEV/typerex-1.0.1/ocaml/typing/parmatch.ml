...
let extra_pat =
  make_pat
    (Tpat_var (Ident.create_with_ctx Ident.predef_ctx "+", mknoloc "+"))
...
let find_label lbl lbls =
  try
    ...
    name
  with Failure "nth" -> Ident.create "*Unkown label*"
...
let rec pretty_val ppf v = match v.pat_desc with
  ...
  | Tpat_construct (_, _, {cstr_tag=tag},[]) ->
      let name = get_constr_name tag v.pat_type v.pat_env in
      fprintf ppf "%s" name
  | Tpat_construct (_, _, {cstr_tag=tag},[w]) ->
      let name = get_constr_name tag v.pat_type v.pat_env in
      fprintf ppf "@[<2>%s@ %a@]" name pretty_arg w
  | Tpat_construct (_, _, {cstr_tag=tag},vs) ->
      let name = get_constr_name tag v.pat_type v.pat_env in
  ...
  | Tpat_alias (v, TPat_alias (x,_)) ->
      fprintf ppf "@[(%a@ as %a)@]" pretty_val v Ident.print x
  | Tpat_or (v,w,_)    ->
      fprintf ppf "@[(%a|@,%a)@]" pretty_or v pretty_or w
  | Tpat_alias (v, TPat_constraint ctyp) -> (* TODO *)
      pretty_val ppf v
  | Tpat_alias (v, TPat_type _) -> (* TODO *)
      pretty_val ppf v

and pretty_car ppf v = match v.pat_desc with
| Tpat_construct (_,_,{cstr_tag=tag}, [_ ; _])
    when is_cons tag v ->
      fprintf ppf "(%a)" pretty_val v
| _ -> pretty_val ppf v

and pretty_cdr ppf v = match v.pat_desc with
| Tpat_construct (_,_,{cstr_tag=tag}, [v1 ; v2])
    when is_cons tag v ->
      fprintf ppf "%a::@,%a" pretty_car v1 pretty_cdr v2
| _ -> pretty_val ppf v
...
| (_,_,{lbl_all=lbl_all},_)::_ ->
    let t =
      Array.map
        (fun lbl -> Path.Pident (Ident.create "?"), mknoloc (Longident.Lident "?"), lbl,omega) lbl_all in
    List.iter
...
let rec normalize_pat q = match q.pat_desc with
  ...
  | Tpat_construct  (lid, lid_loc, c,args) ->
      make_pat (Tpat_construct (lid, lid_loc, c,omega_list args)) q.pat_type q.pat_env
...
| {pat_desc = Tpat_record (omegas,closed)} ->
    ...
    make_pat
      ...
      q.pat_type q.pat_env::
    rest
| {pat_desc = Tpat_construct (lid, lid_loc, c,omegas)} ->
    let args,rest = read_args omegas r in
    make_pat
      (Tpat_construct (lid, lid_loc, c,args)) q.pat_type q.pat_env::
    rest
...
| ({pat_desc = Tpat_construct (_,_,{cstr_tag=Cstr_exception _},_)},_)::_ ->
    false
...
| ({pat_desc = Tpat_construct(_,_,{cstr_tag=(Cstr_constant _|Cstr_block _)},_)} as p,_) :: _ ->
...
| Some ext -> match env with
  | ({pat_desc =
       Tpat_construct(_, _,{cstr_tag=(Cstr_constant _|Cstr_block _)},_)} as p,_)
    :: _ ->
      let path = get_type_path p.pat_type p.pat_env in
      Path.same path ext
  | _ -> false
...
let pat_of_constr ex_pat cstr =
 {ex_pat with pat_desc = Tpat_construct (
   Path.Pident (Ident.create "?"), mknoloc (Longident.Lident "?"),
...
| ({pat_desc = Tpat_construct (lid, lid_loc, ({cstr_tag=Cstr_exception _} as c),_)},_)
  ::_ ->
    make_pat
      (Tpat_construct
         (lid, lid_loc, {c with
...
let rec has_instance p = match p.pat_desc with
  ...
  | Tpat_construct (_, _,_,ps) | Tpat_tuple ps | Tpat_array ps -> has_instances ps
...
| Tpat_construct (lid, lid_loc, c1,ps1), Tpat_construct (_, _,c2,ps2)
      when  c1.cstr_tag = c2.cstr_tag  ->
        let rs = lubs ps1 ps2 in
        make_pat (Tpat_construct (lid, lid_loc, c1,rs)) p.pat_type p.pat_env
  ...
  let conv (typed: Typedtree.pattern) :
    ...
    let rec loop pat =
	  ...
	  List.map
	    ...
	    results
      | Tpat_construct ( cstr_path, cstr_lid, cstr,lst) ->
	  let id = fresh () in
	  Hashtbl.add constrs id (cstr_path, cstr);
	  let results = select (List.map loop lst) in
	  begin match lst with
	    [] ->
	      [mkpat (Ppat_construct(cstr_lid, None, false))]
          | _ ->
	      List.map
	        (fun lst ->
		  let arg =
		  ...
		  in
		  mkpat (Ppat_construct(cstr_lid, arg, false)))
	        results
          end
...
| Tpat_construct (_, _, {cstr_tag=Cstr_exception _}, ps)->
