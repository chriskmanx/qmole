...
*)
let unbound_class = Path.Pident (Ident.create_with_ctx Ident.predef_ctx "")
...
let rec class_field cl_num self_type meths vars
  ...
  | Pcf_let (rec_flag, sdefs) ->
      ...
      let (vals, met_env, par_env) =
        List.fold_right
          (fun (id, sloc) (vals, met_env, par_env) ->
             let expr =
               Typecore.type_exp val_env
                 {pexp_desc = Pexp_ident (mkloc (Longident.Lident (Ident.name id)) sloc.loc);
...
and class_expr cl_num val_env met_env scl =
  ...
  | Pcl_fun (l, Some default, spat, sbody) ->
      ...
      let scases =
        [{ppat_loc = loc; ppat_desc =
          Ppat_construct(mknoloc (Longident.Ldot (Longident.Lident"*predef*", "Some")),
                         Some{ppat_loc = loc; ppat_desc = Ppat_var (mknoloc "*sth*")},
                         false)},
         {pexp_loc = loc; pexp_desc = Pexp_ident(mknoloc (Longident.Lident"*sth*"))};
         {ppat_loc = loc; ppat_desc =
          Ppat_construct(mknoloc (Longident.Ldot (Longident.Lident"*predef*", "None")),
                         None, false)},
         default] in
      let smatch =
  ...
  | Pcl_fun (l, None, spat, scl') ->
      ...
      let pv =
        List.map
          (function (id, id_loc, id', ty) ->
            (id, id_loc,
             Typecore.type_exp val_env'
               {pexp_desc = Pexp_ident (mknoloc (Longident.Lident (Ident.name id)));
                pexp_loc = Location.none}))
          pv
      in
      ...
      rc {cl_desc = Tcl_fun (l, pat, pv, cl, partial);
          cl_loc = scl.pcl_loc;
          cl_type = Cty_fun (l, Ctype.instance pat.pat_type, cl.cl_type);
          cl_env = val_env}
  | Pcl_apply (scl', sargs) ->
      let cl = class_expr cl_num val_env met_env scl' in
