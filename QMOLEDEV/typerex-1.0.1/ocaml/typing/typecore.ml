...
let option_none ty loc =
  ...
  { exp_desc = Texp_construct( path, mknoloc lid, cnone, []);
    exp_type = ty; exp_loc = loc; exp_env = Env.initial }

let option_some texp =
  let lid = Longident.Lident "Some" in
  let (path, csome) = Env.lookup_constructor lid Env.initial in
  { exp_desc = Texp_construct(path, mknoloc lid , csome, [texp]); exp_loc = texp.exp_loc;
...
(* pattern environment *)
let pattern_variables = ref ([]: (Ident.t * type_expr * string loc) list)
...
let enter_variable ?(is_module=false) name ty =
  if List.exists (fun (id, _, _) -> Ident.name id = name.txt) !pattern_variables
  then raise(Error(name.loc, Multiply_bound_variable name.txt));
  let id = Ident.create name.txt in
  pattern_variables := (id, ty, name) :: !pattern_variables;
...
let rec type_pat ~constrs ~labels ~no_existentials ~mode ~env sp expected_ty =
  ...
  match sp.ppat_desc with
    Ppat_any ->
      rp {
        ...
        pat_env = !env }
  | Ppat_var name ->
      let id = enter_variable name expected_ty in
      rp {
        ...
        pat_env = !env }
  | Ppat_unpack name ->
      let id = enter_variable name expected_ty ~is_module:true in
      rp {
        pat_desc = Tpat_var (id, name);
        pat_loc = sp.ppat_loc;
        pat_type = expected_ty;
  ...
  | Ppat_constraint({ppat_desc=Ppat_var name; ppat_loc=loc},
      ...
      | Tpoly (body, tyl) ->
          ...
          end_def ();
          generalize ty';
          let id = enter_variable name ty' in
          rp { pat_desc = Tpat_var (id, name);
               pat_loc = loc;
               pat_type = ty;
               pat_env = !env }
      | _ -> assert false
  ...
  |Ppat_alias(sq, name) ->
      ...
      end_def ();
      generalize ty_var;
      let id = enter_variable name ty_var in
      rp {
        pat_desc = Tpat_alias(q, TPat_alias (id, name));
        pat_loc = loc;
        pat_type = q.pat_type;
        pat_env = !env }
  | Ppat_constant cst ->
  ...
  |Ppat_construct(lid, sarg, explicit_arity) ->
      ...
      rp {
        pat_desc = Tpat_construct( constr_path, lid, constr, args);
        pat_loc = loc;
        pat_type = expected_ty;
        pat_env = !env }
  |Ppat_variant(l, sarg) ->
  ...
  |Ppat_constraint(sp, sty) ->
      let cty, force = Typetexp.transl_simple_type_delayed !env sty in
      unify_pat_types loc !env cty.ctyp_type expected_ty;
  ...
  |Ppat_type lid ->
      ...
      unify_pat_types loc !env ty expected_ty;
      rp {
        pat_desc = Tpat_alias (p, TPat_type (path, lid));
        pat_loc = loc;
        pat_type = p.pat_type;
        pat_env = p.pat_env  }
...
let add_pattern_variables env =
  ...
  (List.fold_right
    (fun (id, ty, name) env ->
       ...
       Env.add_annot id (Annot.Iref_internal name.loc) e1
    )
    pv env,
...
let wrap_unpacks sexp (unpacks : string Asttypes.loc list) =
  List.fold_left
    (fun sexp name ->
...
and type_expect ?in_function env sexp ty_expected =
  ...
  | Pexp_for(param, slow, shigh, dir, sbody) ->
      ...
      let (id, new_env) =
        Env.enter_value param.txt {val_type = instance Predef.type_int;
      ...
      let (arg, ty',cty,cty') =
        ...
        | (Some sty, None) ->
            ...
            if !Clflags.principal then begin
              ...
              (type_argument env sarg ty (instance ty), instance ty, Some cty, None)
            end else
        ...
        | (Some sty, Some sty') ->
            ...
            if !Clflags.principal then begin
              ...
              generalize_structure ty;
              generalize_structure ty';
              (type_argument env sarg ty (instance ty), instance ty', Some cty, Some cty')
            end else
        ...
        (path_self, _) ->
          let type_override (lab, snewval) =
            begin try
              ...
              (Path.Pident id, lab, type_expect env snewval (instance ty))
            with
              Not_found ->
                raise(Error(loc, Unbound_instance_variable lab.txt))
            end
  ...
  | Pexp_open (lid, e) ->
      ...
      re { exp with
        exp_desc = Texp_open (path, lid, exp);
        exp_loc = loc;
        exp_env = env }
...
and type_label_exp create env loc ty_expected
  ...
  in
  (label_path, lid, label, {arg with exp_type = instance arg.exp_type})
...
and type_argument env sarg ty_expected' ty_expected =
  ...
  | {desc = Tarrow("",ty_arg,ty_res,_); level = lv}, _ ->
      ...
      let var_pair name ty =
        ...
        {pat_desc = Tpat_var (id, mknoloc name); pat_type = ty;
         pat_loc = Location.none; pat_env = env},
        {exp_type = ty; exp_loc = Location.none; exp_env = env; exp_desc =
         Texp_ident(Path.Pident id,
                    mknoloc (Longident.Lident name), {val_type = ty; val_kind = Val_reg})}
      in
...
and type_construct env loc lid sarg explicit_arity ty_expected =
  ...
  let texp =
    re {
      exp_desc = Texp_construct(path, lid, constr, []);
      exp_loc = loc;
      exp_type = ty_res;
      exp_env = env } in
  if !Clflags.principal then begin
  ...
  { texp with exp_desc = Texp_construct(path, lid, constr, args)}
