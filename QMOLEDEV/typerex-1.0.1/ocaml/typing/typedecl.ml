...
let make_params sdecl =
  ...
  try
	...
	| Some x ->
	    enter_type_variable true sdecl.ptype_loc x.txt)
      sdecl.ptype_params
  with Already_bound ->
...
let transl_declaration env (name, sdecl) id =
  ...
  let (tkind, kind) =  match sdecl.ptype_kind with
    ...
    | Ptype_variant cstrs ->
            ...
            List.iter
              (fun (name, _, _, _) ->
                if StringSet.mem name.txt !all_constrs then
                  raise(Error(sdecl.ptype_loc, Duplicate_constructor name.txt));
                all_constrs := StringSet.add name.txt !all_constrs)
              cstrs;
            if List.length
        ...
        | Ptype_record lbls ->
            ...
            List.iter
              (fun (name, mut, arg, loc) ->
                if StringSet.mem name.txt !all_labels then
                  raise(Error(sdecl.ptype_loc, Duplicate_label name.txt));
                all_labels := StringSet.add name.txt !all_labels)
              lbls;
...
let transl_type_decl env name_sdecl_list =
  ...
  *)
  ...
  Ctype.begin_def();
  (* Enter types. *)
  let temp_env = List.fold_left2 (fun env (name, sdecl) id ->
    enter_type env (name, sdecl) id
  ) env name_sdecl_list id_list in
