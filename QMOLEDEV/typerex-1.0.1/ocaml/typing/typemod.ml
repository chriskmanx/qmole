...
let sig_item desc typ env loc = {
  sig_desc = desc; sig_loc = loc; sig_env = env
}
  ...
  let rec transl_sig env sg =
	...
	let loc = item.psig_loc in
        ...
        | Psig_value(name, sdesc) ->
            ...
            let (id, newenv) = Env.enter_value name.txt desc env in
        ...
        | Psig_exception(name, sarg) ->
            ...
            let (trem, rem) = transl_sig newenv srem in
            mksig (Tsig_exception (id, name, targ)) env loc :: trem,
...
and type_structure funct_body anchor env sstr scope =
  ...
  let rec type_struct env sstr =
    ...
    | Pstr_primitive(name, sdesc) ->
        ...
        let (id, newenv) = Env.enter_value name.txt desc.val_val env in
        let (str_rem, sig_rem, final_env) = type_struct newenv srem in
	...
	let arg = List.map (fun cty -> cty.ctyp_type) targ in
        let (id, newenv) = Env.enter_exception name.txt arg env in
        let (str_rem, sig_rem, final_env) = type_struct newenv srem in
        (mkstr (Tstr_exception(id, name, targ)) loc :: str_rem,
  ...
  try
    ...
    if !Clflags.print_types then begin
      ...
      if Sys.file_exists sourceintf then begin
        let intf_file =
          try
            find_in_path_uncap !Config.load_path
              (modulename ^ List.hd !Clflags.cmi_exts)
          with Not_found ->
        ...
        if not !Clflags.dont_write_files then
            Env.save_signature simple_sg modulename
              (outputprefix ^ ".cmi");
        (str, coercion)
      end
    end
  with e when !Clflags.save_partial_binannot ->
...
let package_units objfiles cmtifile modulename =
  ...
  let units =
    List.map
      (fun f ->
         ...
         let interface =
           let cmti = pref ^ List.hd !Clflags.cmi_exts in
           if Sys.file_exists cmti then cmti else f
         in
         let sg = Env.read_signature modname interface in
         if Filename.check_suffix f ".cmti" &&
            not(Mtype.no_code_needed_sig Env.initial sg)
         then raise(Error(Location.none, Implementation_is_required f));
         (modname, Env.read_signature modname interface))
      objfiles in
  (* Compute signature of packaged unit *)
  Ident.reinit();
  let sg = package_signatures Subst.identity units in
  (* See if explicit interface is provided *)
  let prefix = chop_extension_if_any cmtifile in
  let mlifile = prefix ^ !Config.interface_suffix in
  if Sys.file_exists mlifile then begin
    if not (Sys.file_exists cmtifile) then begin
      raise(Error(Location.in_file mlifile, Interface_not_compiled mlifile))
    end;
    let dclsig = Env.read_signature modulename cmtifile in
