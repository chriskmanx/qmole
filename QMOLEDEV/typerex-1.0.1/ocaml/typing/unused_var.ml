...
let add_vars tbl (vll1, vll2) =
  let add_var (v, _loc, used) = Hashtbl.add tbl v.txt used in
  List.iter add_var vll1;
  List.iter add_var vll2;
;;

let rm_vars tbl (vll1, vll2) =
  let rm_var (v, _, _) = Hashtbl.remove tbl v.txt in
...
let check_rm_vars ppf tbl (vlul_pat, vlul_as) =
  let check_rm_var kind (v, loc, used) =
    if not !used && not (silent v.txt)
    then Location.print_warning loc ppf (kind v.txt);
    Hashtbl.remove tbl v.txt;
  in
...
let check_rm_let ppf tbl vlulpl =
  let check_rm_one flag (v, loc, used) =
    Hashtbl.remove tbl v.txt;
    flag && (silent v.txt || not !used)
  in
  let warn_var w_kind (v, loc, used) =
    if not (silent v.txt) && not !used
    then Location.print_warning loc ppf (w_kind v.txt)
  in
...
and expression ppf tbl e =
  ...
  | Pexp_ident { txt = Longident.Lident id } ->
