...
#include <arpa/inet.h>
#endif
#ifdef LIBVNCSERVER_HAVE_SYS_UN_H
#include <sys/un.h>
#endif
...
rfbInitSockets(rfbScreenInfoPtr rfbScreen)
{
    ...
    if (rfbScreen->inetdSock != -1) {
        ...
        if(!rfbSetTcpNoDelay(rfbScreen->inetdSock))
	    return;
...
rfbCheckFds(rfbScreenInfoPtr rfbScreen,long usec)
{
	...
	if (rfbScreen->listenSock != -1 && FD_ISSET(rfbScreen->listenSock, &fds)) {
...
#ifdef USE_LIBWRAP
	    if(sockIsInet && !hosts_ctl("vnc",STRING_UNKNOWN,inet_ntoa(addr.sin_addr),
			STRING_UNKNOWN)) {
		...
		sockIsInet ? inet_ntoa(addr.sin_addr) : "non-AF_INET-socket");
...
int
rfbListenOnUnixSocket(char *file, rfbScreenInfoPtr rfbScreen)
{
#ifdef WIN32
    rfbLog("Windows doesn't support UNIX sockets\n");
    return -1;
#else
    int sock;
    ...
    if (rfbScreen) {
	if (rfbScreen->socketState != RFB_SOCKET_READY) {
	    rfbLog("rfbScreen is not in RFB_SOCKET_READY state.\n");
	    return -1;
	} else if (rfbScreen->inetdSock >= 0) {
	    rfbLog("Cannot have inetdSock and listenSock simultaneously.\n");
	    return -1;
	}
    }

    if (!file || strlen(file)+1 > sizeof(addr.sun_path)) {
	rfbLog("rfbListenOnUnixSocket: invalid filename '%s'.\n",
	    file ? file : "null");
	return -1;
    }
    ...
    if (sock < 0) {
	rfbLogPerror("rfbListenOnUnixSocket: socket failed");
	return -1;
    }
    ...
    if (bind(sock, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
	rfbLogPerror("rfbListenOnUnixSocket: bind failed");
	closesocket(sock);
	return -1;
    }

    if (listen(sock, 32) < 0) {
	rfbLogPerror("rfbListenOnUnixSocket: listen failed");
	closesocket(sock);
	return -1;
    }
	...
	if (rfbScreen->listenSock >= 0) {
	    ...
	    FD_CLR(rfbScreen->listenSock, &rfbScreen->allFds);
	    rfbScreen->listenSock = -1;
	    rfbLog("Switched to listening on Unix Socket: %s\n", file);
	} else {
	    rfbLog("Listening on Unix Socket: %s\n", file);
	}
	rfbScreen->listenSock = sock;
	FD_SET(rfbScreen->listenSock, &(rfbScreen->allFds));
	if (rfbScreen->listenSock > rfbScreen->maxFd) {
	    rfbScreen->maxFd = rfbScreen->listenSock;
	}
    }
    return sock;
...
#else
    struct sockaddr_un addr;
    int fd, fromlen = sizeof(addr);

    fd = accept(sock, (struct sockaddr *)&addr, &fromlen);
    if (fd < 0) {
	rfbLogPerror("rfbAcceptUnixSocket: accept failed");
	return -1;
    }
    return fd;
#endif
...
/*
 * rfbSetTcpNoDelay sets a TCP socket into TCP_NODELAY mode,
 * and leaves other file descriptors alone. 
 */
rfbBool
rfbSetTcpNoDelay(int sock)
{
	...
	if (setsockopt(sock, IPPROTO_TCP, TCP_NODELAY,
		       (char *)&one, sizeof(one)) < 0) {
	    rfbLogPerror("setsockopt TCP_NODELAY");
	    return FALSE;
	}
    }
    return TRUE;
