...
int cursor_noshape_updates_clients(rfbScreenInfoPtr s) {
	...
	if (skip < 0) {
		skip = 0;
		/* for testing non-CopyRect clients... e.g. old VNC2Go */
		if (getenv("X11VNC_SKIP_cursor_noshape_updates_clients")) skip = 1;
	}
	if (skip) {
		return 0;
	}
	...
	rfbReleaseClientIterator(iter);
	if (calls++ < 5) {
		rfbLog("cursor_noshape_updates_clients: %d\n", count);
	}
	return count;
}
...
void cursor_position(int x, int y) {
	...
	if (debug_pointer > 1) rfbLog("cursor_position(1): x: %d y: %d\n", x, y);
	...
	if (scaling) {
		...
		y = nfix(y, scaled_y);
		if (debug_pointer > 1) rfbLog("cursor_position(2): x: %d y: %d dpy_x: %d dpy_y: %d scaled_x: %d scaled_y: %d\n", x, y, dpy_x, dpy_y, scaled_x, scaled_y);
	}

	if (clipshift) {
		int udpy_x = scaling ? scaled_x : dpy_x;
		int udpy_y = scaling ? scaled_y : dpy_y;

		if (x < 0) x = 0;
		if (y < 0) y = 0;
		if (x >= udpy_x) x = udpy_x-1;
		if (y >= udpy_y) y = udpy_y-1;
		if (debug_pointer > 1) rfbLog("cursor_position(3): x: %d y: %d dpy_x: %d dpy_y: %d udpy_x: %d udpy_y: %d\n", x, y, dpy_x, dpy_y, udpy_x, udpy_y);
	}
...
if (0) fprintf(stderr, "check_x11_pointer %d %d\n", root_x, root_y);
	...
	if (debug_pointer) {
		...
		if (root_x != last_x || root_y != last_y) {
			rfbLog("XQueryPointer:     x:%4d, y:%4d\n",
			    root_x, root_y);
			if (debug_pointer > 1) {
				rfbLog("check_x11_pointer x:%4d y:%4d "
				    "d: %dx%d+%d+%d wd: %dx%d cd: %dx%d+%d+%d\n",
				    root_x - off_x - coff_x, root_y - off_y - coff_y, dpy_x, dpy_y, off_x, off_y, wdpy_x, wdpy_y, cdpy_x, cdpy_y, coff_x, coff_y);
			}
		}
		last_x = root_x;
		last_y = root_y;
	...
	if (clipshift) {
		...
		if (x < 0 || y < 0 || x >= dpy_x || y >= dpy_y)  {
			if (cnt++ % 4 != 0) {
				if (debug_pointer) {
					rfbLog("Skipping cursor_position() x:%4d y:%4d outside our "
					    "clipshift dpy_x/y: %d/%d wdpy_x/y: %d/%d cdpy_x/y: %d/%d\n",
					    x, y, dpy_x, dpy_y, wdpy_x, wdpy_y, cdpy_x, cdpy_y);
				}
			}
			return 0;
		}
