...
int listen_unix(char *file) {
	...
	rfbLog("listen_unix: AF_UNIX socket %s\n", file ? file : "null");

	rfbLog("listen_unix: warning: libvnserver does not support AF_UNIX sockets.\n");

	if (!file || strlen(file)+1 > sizeof(addr.sun_path)) {
		rfbLog("listen_unix: filename too long: %s\n", file);
		return -1;
	}
	...
	rfbLog("linux_unix: listening on unix socket: %s fd=%d\n", file, sock);
	return sock;
#else
	int sock = rfbListenOnUnixSocket(file, NULL);
...
int accept_unix(int sock) {
	...
	rfbLog("accept_unix: warning: libvnserver does not support AF_UNIX sockets.\n");
	...
	return fd;
#else
	int fd = rfbAcceptUnixSocket(sock);
	rfbLog("accept_unix: rfbAcceptUnixSocket(%d) returned fd=%d\n", sock, fd);
	return fd;
#endif
