...
void macosx_copy_opengl(char *dest, int x, int y, unsigned int w, unsigned int h) {
	...
	if (err_after != GL_NO_ERROR) {
		...
		if (err_msg < pr_max) {
			err_msg++;
			rfbLog("macosx_copy_opengl: %05d ERR:  err_before: 0x%x -> err_after: 0x%x dest: %p geom: %dx%d+%d+%d\n", err_msg, (int) err_before, (int) err_after, dest, w, h, x, y);
		}
		...
		if (pr) {
			rfbLog("macosx_copy_opengl: %05d info: err_before: 0x%x -> err_after: 0x%x dest: %p geom: %dx%d+%d+%d\n", dbg_msg, (int) err_before, (int) err_after, dest, w, h, x, y);
		}
		dbg_msg++;
	}
