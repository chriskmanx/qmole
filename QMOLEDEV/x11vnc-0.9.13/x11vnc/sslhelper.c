...
void accept_openssl(int mode, int presock) {
	int sock = -1, listen = -1, cport = -1, csock = -1, vsock = -1;	
	int peerport = 0;
	int status, n, i, db = 0;
	...
	ret = -2;
#ifdef LIBVNCSERVER_ALLOW_NON_AF_INET_SOCKETS
	...
	if (ret < 0) {
		if (ret != -2) {
			rfbLogPerror("socketpair");
			rfbLog("accept_openssl: socketpair failed, using INADDR_LOOPBACK socket\n");
		}
		...
		rfbLog("accept_openssl: using socketpair: %d %d\n", sockpair[0], sockpair[1]);
	}
	...
	if (client) {
		...
		if (swt) {
			if (client->host) {
				rfbLog("accept_openssl: old client name: '%s'\n", client->host);
				free(client->host);
			}
			client->host = strdup(openssl_last_ip);
			rfbLog("accept_openssl: new client name: '%s'\n", client->host);
		}
		...
		if (openssl_last_ip &&
			...
			rfbLog("accept_openssl: renaming client '%s' -> '%s'\n",
			    client->host, openssl_last_ip);
			client->host = strdup(openssl_last_ip);
		}
		if (vencrypt_sel != 0) {
