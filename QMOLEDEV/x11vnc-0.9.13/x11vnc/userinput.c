...
void do_copyregion(sraRegionPtr region, int dx, int dy, int mode)  {
	...
	static int db = -1, forceCR = 0;
	if (db < 0) {
		db = 0;
		if (getenv("X11VNC_DEBUG_COPYRECT")) db = 1;
		if (getenv("X11VNC_FORCE_COPYRECT")) forceCR = 1;
	}

	last_copyrect = dnow();

if (db || debug_scroll > 1) fprintf(stderr, "Start: do_copyregion(dx: %d dy: %d mode: %d)\n", dx, dy, mode); 

	if (rfb_fb == main_fb && ! rotating && mode == DCR_Normal) {
		/* normal case, no -scale or -8to24 */
		get_client_regions(&req, &mod, &cpy, &ncli);

if (db || debug_scroll > 1) fprintf(stderr, ">>>-rfbDoCopyRect req: %d mod: %d cpy: %d\n", req, mod, cpy); 
...
if (db || debug_scroll > 1) fprintf(stderr, "<<<-rfbDoCopyRect req: %d mod: %d cpy: %d\n", req, mod, cpy); 
if (db || debug_scroll > 1) fprintf(stderr, "do_copyregion(): NORMAL case done: returning.\n"); 
...
if (db) fprintf(stderr, "copyrect: cmap8to24_fb: mode=%d\n", mode);
				if (cmap8to24) {
					if (depth <= 8) {
						Bpp    = 4 * Bpp0;
						stride = 4 * stride0;
...
if (db) fprintf(stderr, "sa.. %d %d %d %d %d %d\n", sx1, sy1, sx2, sy2, sdx, sdy);
...
if (db) fprintf(stderr, "sb.. %d %d %d %d %d %d\n", sx1, sy1, sx2, sy2, sdx, sdy);

		if (mode == DCR_Direct) {
			rfbClientIteratorPtr i;
			rfbClientPtr cl;
			sraRegionPtr r = sraRgnCreateRect(sx1, sy1, sx2, sy2);
			int nonCR = 0, smallReq = 0;
			...
			while( (cl = rfbClientIteratorNext(i)) ) {
				if (!cl->useCopyRect) {
					...
					sraRegionPtr all = sraRgnCreateRect(0, 0, screen->width, screen->height);
					sraRgnSubtract(all, cl->requestedRegion);
					if (!sraRgnEmpty(all)) {
						smallReq++;
					}
					sraRgnDestroy(all);
			...
			if (db) rfbLog("nonCR=%d smallReq=%d\n", nonCR, smallReq);
			if ((nonCR || smallReq) && !forceCR) {
	...
	sraRgnReleaseIterator(iter);
if (db || debug_scroll > 1) fprintf(stderr, "do_copyregion(): TRICKY case done: returning.\n"); 
}
...
if (db || debug_scroll > 1) fprintf(stderr, "batch_copyregion(ncr: %d dx[0]: %d dy[0]: %d)\n", ncr, dx[0], dy[0]); 
	...
	for (k=0; k < ncr; k++) {
		...
		while (sraRgnIteratorNext(iter, &rect)) {
			...
			if (x1 > xm || y1 > ym || x2 > xm || y2 > ym) {
				if (db || ncdb) fprintf(stderr, "batch_copyregion: BAD RECTANGLE: %d,%d %d,%d\n", x1, y1, x2, y2);
				bad = 1;
			}
	...
	if ((nonCR || smallReq) && !forceCR) {
		for (k=0; k < ncr; k++) {
			do_copyregion(region[k], dx[k], dy[k], DCR_Normal);
		}
		return;
	}
	if (bad) {
