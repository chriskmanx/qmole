...
typedef struct _NBNS_LABEL_LENGTH
{
    uint8_t len;
} NBNSLabelLength;

typedef struct _NBNS_LABEL_DATA
{
    uint8_t len;
...
typedef struct _NBNS_LABEL
{
    uint16_t type;
    uint16_t class_id;
} NBNSLabel;

typedef struct _NBNS_LABEL_POINTER
{
    uint8_t flag;
    uint8_t position;
} NBNSLabelPtr;

typedef struct _NBNS_ANSWER_DATA
{
    uint32_t ttl;
    uint16_t data_len;
} NBNSAnswerData;

typedef struct _NBSS_HEADER
{
    uint8_t type;
    uint8_t flags;
    uint16_t length;
...
static uint8_t NB_SMB_BANNER[] =
{
    0xFF, 'S', 'M', 'B'
};

typedef struct _SERVICE_SMB_HEADER
{
    uint8_t command;
    uint32_t status;
    uint8_t flags[3];
...
typedef struct _SERVICE_SMB_ANDX_RESPONSE
{
    uint8_t wc;
    uint8_t cmd;
    uint8_t reserved;
    ...
    uint16_t action;
    uint16_t sec_len;
} ServiceSMBAndXResponse;

typedef struct _SERVICE_SMB_NEGOTIATE_PROTOCOL_RESPONSE
{
    uint8_t wc;
    ...
    uint16_t time_zone;
    uint8_t sec_len;
} ServiceSMBNegotiateProtocolResponse;

typedef struct _SERVICE_SMB_TRANSACTION_HEADER
{
    uint8_t wc;
    uint16_t total_pc;
    uint16_t total_dc;
...
typedef struct _SERVICE_SMB_BROWSER_HEADER
{
    uint8_t command;
    uint8_t count;
    uint32_t period;
...
} ServiceSMBBrowserHeader;

typedef struct _SERVICE_NBSS_DATA
{
    NBSSState state;
    unsigned count;
    uint32_t length;
...
{
    {&nbns_validate, 137, IPPROTO_TCP},
    {&nbns_validate, 137, IPPROTO_UDP},
    {&nbns_validate, 137, IPPROTO_UDP, 1},
    {&nbdgm_validate, 138, IPPROTO_UDP},
    {&nbss_validate, 139, IPPROTO_TCP},
    {&nbss_validate, 445, IPPROTO_TCP},
    {NULL, 0, 0}
};
...
static int netbios_init(const InitServiceAPI * const init_api)
{
    init_api->RegisterPattern(&nbss_validate, IPPROTO_TCP, NB_SMB_BANNER, sizeof(NB_SMB_BANNER), -1, "netbios");

    _dpd.debugMsg(DEBUG_LOG,"registering appId: %d for NetBIOS-ns\n",APP_ID_NETBIOS_NS);
    init_api->RegisterAppId(&nbns_validate, APP_ID_NETBIOS_NS, APPINFO_FLAG_SERVICE_UDP_REVERSED, NULL);

    _dpd.debugMsg(DEBUG_LOG,"registering appId: %d for NetBIOS-dgm\n",APP_ID_NETBIOS_DGM);
    init_api->RegisterAppId(&nbdgm_validate, APP_ID_NETBIOS_DGM, APPINFO_FLAG_SERVICE_ADDITIONAL, NULL);

    _dpd.debugMsg(DEBUG_LOG,"registering appId: %d for NetBIOS-ssn\n",APP_ID_NETBIOS_SSN);
    init_api->RegisterAppId(&nbss_validate, APP_ID_NETBIOS_SSN, APPINFO_FLAG_SERVICE_ADDITIONAL, NULL);

    _dpd.debugMsg(DEBUG_LOG,"registering appId: %d\n",APP_ID_DCE_RPC);
    init_api->RegisterAppId(&nbss_validate, APP_ID_DCE_RPC, 0, NULL);
                                            ...
                                            char *name)
{
    ...
    if (end - *data < (int)sizeof(NBNSLabelLength)) return -1;
    lbl_len = (NBNSLabelLength *)(*data);
    switch (lbl_len->len & NBNS_LENGTH_FLAGS)
    {
    case 0x00:
        lbl_data = (NBNSLabelData *)(*data);
        if (end - *data < (int)sizeof(NBNSLabelData)) return -1;
    ...
    if (lbl_data->len != NBNS_NAME_LEN) return -1;
    if (lbl_data->zero) return -1;
                                  ...
                                  const uint8_t * const end)
{
    ...
    if (end - *data < (int)sizeof(NBNSLabel)) return -1;
    lbl = (NBNSLabel *)(*data);
    *data += sizeof(NBNSLabel);
    tmp = ntohs(lbl->type);
    if (tmp != NBNS_NB && tmp != NBNS_NBSTAT) return -1;
    return 0;
}
                                ...
                                const uint8_t * const end)
{
    ...
    if (!ret)
    {
        ...
        if (end - *data < (int)sizeof(NBNSAnswerData)) return -1;
        *data += sizeof(NBNSAnswerData);
        tmp = ntohs(ad->data_len);
        if (end - *data < tmp) return -1;
        *data += tmp;
    }
    return ret;
}
...
MakeRNAServiceValidationPrototype(nbns_validate)
{
    ...
    if (hdr->trunc) goto not_compatible;
    ...
    if (!hdr->response)
    {
        ...
        {
            if (flow_checkflag(flowp, FLOW_UDP_REVERSED)) goto success;
            goto fail;
        }
        goto inprocess;
    }

    if (hdr->replycode > NBNS_REPLYCODE_MAX) goto fail;
...
fail:
    netbios_service_mod.api->fail_service(flowp, pkt, dir, &nbns_svc_element);
    return SERVICE_NOMATCH;

not_compatible:
    netbios_service_mod.api->incompatible_data(flowp, pkt, dir, &nbns_svc_element);
...
static inline void smb_find_domain(const uint8_t *data, uint16_t size, const int dir,
                                   FLOW *flowp, const SFSnortPacket *pkt)
{
    ...
    if (size < sizeof(*smb) + sizeof(wc)) return;
    smb = (ServiceSMBHeader *)data;
    if (smb->status != SERVICE_SMB_STATUS_SUCCESS) return;
    if (!(smb->flags[0] & SERVICE_SMB_FLAGS_RESPONSE)) return;
    ...
    else if (smb->command == SERVICE_SMB_COMMAND_NEGOTIATE_PROTOCOL)
    {
        if (wc == 34)
        {
            ...
            if (capabilities & SERVICE_SMB_CAPABILITIES_EXTENDED_SECURITY) return;
            unicode = (capabilities & SERVICE_SMB_CAPABILITIES_UNICODE) || unicode;
        }
        else if (wc != 26) return;
        if (np->sec_len >= byte_count) return;
        data += np->sec_len;
            ...
            else
            {
                ...
                if (*data != 0)
                {
                    ...
                    _dpd.errMsg( "Failed command %02X  %u  0x%08X:%u->0x%08X:%u",
                           smb->command, byte_count, pkt->src_ip.s_addr, pkt->src_port, pkt->dst_ip.s_addr, pkt->dst_port);
#endif
                    return;
                }
                data++;
    ...
    if (pos)
    {
        ...
        _dpd.debugMsg(DEBUG_LOG, "Found domain %s for command %02X 0x%08X:%u->0x%08X:%u",
                domain, smb->command, pkt->src_ip.s_addr, pkt->src_port, pkt->dst_ip.s_addr, pkt->dst_port);
#endif
        if (!flowp->netbiosDomain)
        {
            flowp->netbiosDomain = strdup(domain);
            if (!flowp->netbiosDomain)
                _dpd.errMsg("failed to allocate netbios domain name");
        }
    }
}
...
MakeRNAServiceValidationPrototype(nbss_validate)
{
    ...
    nd = netbios_service_mod.api->data_get(flowp);
    if (!nd)
    {
        nd = calloc(1, sizeof(*nd));
        if (!nd)
            return SERVICE_ENOMEM;
        if (netbios_service_mod.api->data_add(flowp, nd, &nbss_free_state))
        {
            free(nd);
            return SERVICE_ENOMEM;
        }
    ...
    while (data < end)
    {
        ...
        case NBSS_STATE_CONNECTION:
            if (size < sizeof(NBSSHeader)) goto fail;
            hdr = (NBSSHeader *)data;
            ...
            case NBSS_TYPE_RESP_POSITIVE:
                if (hdr->flags || hdr->length) goto fail;
                nd->state = NBSS_STATE_FLOW;
                break;
            case NBSS_TYPE_RESP_NEGATIVE:
                if (hdr->flags || ntohs(hdr->length) != 1) goto fail;
            ...
            case NBSS_TYPE_RESP_RETARGET:
                if (hdr->flags || ntohs(hdr->length) != 6) goto fail;
                if (end - data < 6) goto fail;
                data += 6;
                break;
            default:
                goto fail;
...
MakeRNAServiceValidationPrototype(nbdgm_validate)
{
    ...
    if (hdr->zero) goto fail;
    if (!hdr->first || hdr->more) goto fail;
    ...
    case NBDGM_TYPE_REQUEST:
        if (netbios_validate_name(&data, data, end)) goto fail;
        if (end != data) goto fail;
        goto success;
    ...
    case NBDGM_TYPE_BROADCAST:
        ...
        if (data >= end) goto fail;
        if (netbios_validate_name_and_decode(&data, data, end, source_name)) goto fail;
        if (data >= end) goto fail;
        if (netbios_validate_name(&data, data, end)) goto fail;
        if (data >= end) goto fail;
            ...
            !memcmp(data, NB_SMB_BANNER, sizeof(NB_SMB_BANNER)))
        {
            ...
            if (end-data < (int)sizeof(ServiceSMBHeader)) goto not_mailslot;
            smb = (ServiceSMBHeader *)data;
            data += sizeof(ServiceSMBHeader);
            if (smb->command != SERVICE_SMB_TRANSACTION_COMMAND) goto not_mailslot;
            if (end-data < (int)sizeof(ServiceSMBTransactionHeader)) goto not_mailslot;
            trans = (ServiceSMBTransactionHeader *)data;
            data += sizeof(ServiceSMBTransactionHeader);
            if (trans->wc == SERVICE_SMB_NOT_TRANSACTION_WC) goto not_mailslot;
            if ((unsigned)(end-data) < (trans->sc*2)+sizeof(uint16_t)+sizeof(mailslot)+sizeof(ServiceSMBBrowserHeader))
                goto not_mailslot;
            data += (trans->sc*2);
            len = *((uint16_t *)data);
            data += sizeof(uint16_t);
            if (end-data < len) goto not_mailslot;
            if (memcmp(data, mailslot, sizeof(mailslot))) goto not_mailslot;
