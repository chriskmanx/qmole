...
typedef struct _session_api
{
   ...
   /* Turn off inspection for potential session.
     ...
     *     -1 on failure
     */
    int (*ignore_session)(const SFSnortPacket *, snort_ip_p, uint16_t, snort_ip_p, uint16_t, uint8_t, uint32_t, char, char);
    ...
    */
     ...
     *     -1 on failure
     */
    int (*set_application_protocol_id_expected)(const SFSnortPacket *, snort_ip_p, uint16_t, snort_ip_p, uint16_t,
